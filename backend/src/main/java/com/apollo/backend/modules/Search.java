...
public class Search implements RamaModule {
        ...
        public static MatchInfo numStatusTermMatches(Object record, String searchTerm, List<String> allTerms) {
                ...
                for (String term : allTerms) {
                        if (info.getTerms().contains(term))
                                matches++;
                }
                return new MatchInfo(new ArrayList() {
                        {
                                add(info.getAccountId());
                                add(info.getStatusId());
                        }
                }, matches);
        }
        ...
        // a full match.
        public static Map searchForMatches(RamaFunction3<Object, String, List, MatchInfo> termMatchesFn, SortedMap m,
                        String searchTerm, List<String> allTerms, int pageAmount, int limit) {
                ...
                Long nextId = null;
                if (matches > limit)
                        nextId = lastMatchingId;
                else if (m.size() == pageAmount)
                        nextId = (Long) m.lastKey();
                ret.put("nextId", nextId);
                return ret;
        }

        private Block finishProfileTermIndexing(KeyToFixedItemsPStateGroup profileTerms, String nameVar,
                ...
                return Block.each((String username, String displayName) -> {
                        ...
                        if (displayName != null) {
                                for (String term : displayName.toLowerCase().split(" ")) {
                                        if (!term.isEmpty())
                                                ret = (PersistentHashSet) ret.cons(term);
                                }
                        }
                        username = username.toLowerCase();
                        ret = (PersistentHashSet) ret.cons(username);
                        if (username.length() > 5)
                                ret = (PersistentHashSet) ret.cons(username.substring(0, 5));
                        return ret;
                }, nameVar, displayNameVar).out(termSetVar)
                                .each(Ops.EXPLODE, termSetVar).out(termVar)
                                .hashPartition(termVar)
                                .each((PersistentHashSet termSet, String term,
                                                String username) -> new ProfileSearchRecord(
                                                                (Set) termSet.disjoin(term)
                                                                                .disjoin(username.toLowerCase()),
                                                                username),
                                                termSetVar, termVar, nameVar)
                                .out(profileSearchRecordVar)
        ...
        // - Returns map containing "term", "matchList" (at most "*limit"), "nextId",
        // "termMatches".
        // - this macro is coded for conciseness by taking advantage that it's supposed
        // to be the
        // entire query implementation
        // - as such it doesn't generate any intermediate vars
        // - instead, other vars used by caller (e.g. for partitioner function) should
        // be generated
        // - this macro requires vars called "*terms" and "*limit" to be in scope
        private Block bestTermSearchQuery(String pstateVar, RamaFunction2<String, String, Block> partitionerMacro,
                        RamaFunction3<Object, String, List, MatchInfo> itemTermMatches) {
                return Block.each(Ops.SIZE, "*terms").out("*numTerms")
                                .loopWithVars(LoopVars.var("*i", 0)
                                                ...
                                                Block.each(Ops.GET, "*terms", "*i").out("*term")
                                                                ...
                                                                .localSelect(pstateVar, Path.key("*queryKey")
                                                                                .sortedMapRangeFrom(0, pageAmount))
                                                                .out("*infoMap")
                                                                .each(Search::searchForMatches,
                                                                                new Constant(itemTermMatches),
                                                                                "*infoMap", "*term",
                                                                                "*terms", pageAmount, "*limit")
                                                                .out("*info")
                                                                .each((Map best, Map curr) -> {
                                                                        if (best == null)
                                                                                return curr;
                                                                        else {
                                                                                int bestMatches = (int) best
                                                                                                .get("totalMatches");
                                                                                int bestTermMatches = (int) best
                                                                                                .get("termMatches");
                                                                                int bestTermLength = ((String) best
                                                                                                .get("term")).length();
                                                                                int currMatches = (int) curr
                                                                                                .get("totalMatches");
                                                                                int currTermMatches = (int) curr
                                                                                                .get("termMatches");
                                                                                int currTermLength = ((String) curr
                                                                                                .get("term")).length();
                                                                                if (bestMatches > currMatches)
                                                                                        return best;
                                                                                else if (currMatches > bestMatches)
                                                                                        return curr;
                                                                                else if (bestTermMatches > currTermMatches)
                                                                                        return best;
                                                                                else if (currTermMatches > bestTermMatches)
                                                                                        return curr;
                                                                                else if (bestTermLength > currTermLength)
                                                                                        return best;
                                                                                else
                                                                                        return curr;
                                                                        }
                                                                }, "*best", "*info").out("*nextBest")
                                                                .each(Ops.SIZE, new Expr(Ops.GET, "*nextBest",
                                                                                "matchList"))
                                                                .out("*numMatches")
                                                                .ifTrue(new Expr(Ops.EQUAL, "*i",
                                                                                new Expr(Ops.DEC, "*numTerms")),
        ...
        // - See notes for bestTermSearchQuery regarding unusual way this macro is
        // structured
        // - requires "*terms", "*startParams", and "*limit" to be in scope
        private Block termsSearchQuery(String pstateVar, RamaFunction1<String, Block> bestTermSearchMacro,
                        RamaFunction2<String, String, Block> partitionerMacro,
                        RamaFunction3<Object, String, List, MatchInfo> itemTermMatches) {
                return Block.ifTrue(new Expr(Ops.IS_NULL, "*startParams"),
                                ...
                                .each(Ops.OR, new Expr(Ops.GET, "*params", "matchList"), new Expr(ArrayList::new))
                                .out("*matchList")
                                ...
                                .loopWithVars(LoopVars.var("*id", "*startId"),
                                                Block.ifTrue(
                                                                ...
                                                                Block.localSelect(pstateVar, Path.key("*queryKey")
                                                                                .sortedMapRangeFrom("*id",
                                                                                                SortedRangeFromOptions
                                                                                                                .excludeStart()
                                                                                                                .maxAmt(pageAmount)))
                                                                                .out("*infoMap")
                                                                                .each(Ops.MINUS, "*limit", new Expr(
                                                                                                Ops.SIZE, "*matchList"))
                                                                                .out("*iterLimit")
                                                                                .each(Search::searchForMatches,
                                                                                                new Constant(itemTermMatches),
                                                                                                ...
                                                                                                "*iterLimit")
                                                                                .out("*m")
                                                                                .each(Ops.GET, "*m", "nextId")
                                                                                .out("*nextId")
                                                                                .each(Ops.GET, "*m", "matchList")
                                                                                .out("*newMatchList")
                                                                                .each((List ret, List newMatchList) -> ret
                                                                                                .addAll(newMatchList),
                                                                                                "*matchList",
                                                                                                "*newMatchList")
        ...
        private static PersistentVector prependId(List<Long> v, long id, int maxSize, Boolean shouldInclude) {
                if (shouldInclude != null && shouldInclude) {
                        ...
                        return PersistentVector.create(
                                        v.stream().filter((Long aid) -> aid != id).collect(Collectors.toList()));
                }
        }
        ...
        private static PersistentVector removeId(List<List<Long>> v, long id) {
                return PersistentVector
                                .create(v.stream().filter((List<Long> tuple) -> tuple.get(0) != id)
        ...
        public void define(Setup setup, Topologies topologies) {
                ...
                KeyToFixedItemsPStateGroup statusTerms = new KeyToFixedItemsPStateGroup("$$statusTerms", 1000,
                                String.class,
                                StatusSearchRecord.class);
                statusTerms.declarePStates(search);

                // key is term or prefix
                KeyToFixedItemsPStateGroup profileTerms = new KeyToFixedItemsPStateGroup("$$profileTerms", 10000,
                                String.class,
                                ProfileSearchRecord.class);
                profileTerms.declarePStates(search);

                // hashtag prefix -> full hashtags
                KeyToUniqueFixedItemsPStateGroup hashtagPrefixes = new KeyToUniqueFixedItemsPStateGroup(
                                "$$hashtagPrefixes",
                ...
                search.source("*statusWithIdDepot").out("*microbatch")
                                // update directories
                                .batchBlock(
                                                Block.subBatch(filterStatusWithIdSubBatch("*microbatch"))
                                                                .out("*statusWithIds")
                                                                .each((List<StatusWithId> swids) -> swids.stream()
                                                                                .map(swid -> swid.status.authorId)
                                                                                .distinct()
                                                                                .collect(Collectors.toList()),
                                                                                "*statusWithIds")
                                                                .out("*allAccountIds")
                                                                ...
                                                                .localTransform("$$activeAccountIds",
                                                                                Path.term(Search::prependIds,
                                                                                                "*discoverableAccountIds",
                                                                                                maxDirectorySize)))

                                .explodeMicrobatch("*microbatch").out("*statusWithId")
                                .keepTrue(new Expr(Ops.OR,
                                                new Expr(Ops.IS_INSTANCE_OF, EditStatus.class, "*statusWithId"),
                                ...
                                .each((String text, OutputCollector collector) -> {
                                        for (String mention : Token.filterMentions(Token.parseTokens(text)))
                                                collector.emit(mention);
                                }, "*text").out("*targetUsername")
                                .select("$$nameToUser", Path.key("*targetUsername", "accountId"))
                                .out("*targetAccountId")
                                ...
                                .each((String text, Long authorId, Long statusId) -> {
                                        ...
                                        for (Token t : tokens)
                                                ret = (IPersistentSet) ret.cons(t.getOrigContent().toLowerCase());
                                        return new StatusSearchRecord((Set<String>) ret, authorId, statusId);
                                }, "*text", "*authorId", "*statusId").out("*statusSearchRecord")
                                .macro(statusTerms.addItem(new Expr(Object::toString, "*targetAccountId"),
                                                "*statusSearchRecord"));
                ...
                search.source("*accountWithIdDepot").out("*microbatch")
                                ...
                                .macro(extractFields("*account", "*name", "*timestamp", "*displayName",
                                                "*discoverable"))
                                .atomicBlock(Block.macro(
                                                finishProfileTermIndexing(profileTerms, "*name", "*displayName")))
                                // update directories
                                .globalPartition()
                                // update active account directory
                                .localTransform("$$activeAccountIds",
                                                Path.term(Search::prependId, "*accountId", maxDirectorySize,
                                                                "*discoverable"))
                ...
                search.source("*accountEditDepot").out("*microbatch")
                                ...
                                .macro(extractFields("*account", "*name", "*timestamp"))
                                // index display name
                                .each((EditAccount e) -> e.getEdits().stream()
                                                .filter(EditAccountField::isSetDisplayName).findFirst()
                                                .map(TUnion::getFieldValue).orElse(null),
                                                "*editAccount")
                                .out("*editDisplayName")
                                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*editDisplayName"),
                                                Block.atomicBlock(
                                                                Block.macro(finishProfileTermIndexing(profileTerms,
                                                                                "*name", "*editDisplayName"))))
                                // update directories
                                .each((EditAccount e) -> e.getEdits().stream()
                                                .filter(EditAccountField::isSetDiscoverable).findFirst()
                                                .map(TUnion::getFieldValue).orElse(null),
                                                "*editAccount")
                                ...
                                .ifTrue("*editDiscoverable",
                                                Block.each(Ops.TUPLE, "*accountId", "*timestamp").out("*tuple")
                                                                .agg("$$newAccountIds",
                                                                                Agg.topMonotonic(maxDirectorySize,
                                                                                                "*tuple")
                ...
                search.source("*reviewHashtagDepot").out("*microbatch")
                                ...
                                .subSource("*data",
                                                SubSource.create(ReviewItem.class)
                                                                ...
                                                                .hashPartition("*prefix")
                                                                .macro(reviewedHashtagPrefixes.addItem("*prefix",
                                                                                "*item")),
                ...
                topologies.query("statusTermsSearch", accountIdVar, "*terms", "*startParams", "*limit").out("*info")
                                .macro(termsSearchQuery(
                                                ...
                                                (String paramsVar) -> Block
                                                                .each(Search::basicSearchStartingInfo, accountIdVar)
                                                                .out(paramsVar),
                                                (String termVar, String keyVar) -> Block.hashPartition(accountIdVar)
                ...
                topologies.query("bestProfileTermSearch", "*terms", "*limit").out("*info")
                                .macro(bestTermSearchQuery(
                                                "$$profileTerms",
                                                (String termVar, String keyVar) -> Block.hashPartition(termVar)
                                                                .each(Ops.IDENTITY, termVar).out(keyVar),
                                                Search::numProfileTermMatches));

                // - Returns map with "term", "nextId", "matchList".
                // - NOTE: display name could have changed since the ProfileSearchRecord was
                // indexed,
                // so this could return outdated matches. This could be extended to verify
                // username's still match with current display name, but Mastodon does not
                // appear to
                // do so and will sometimes return outdated records that don't currently match.
                // (People
                // change their display names infrequently enough that this isn't a concern)
                // - NOTE: this doesn't search for exact match since it doesn't fit the
                // character of this
                ...
                topologies.query("profileTermsSearch", "*inputTerms", "*startParams", "*limit").out("*info")
                                .each((List<String> terms) -> {
                                        ...
                                        if (term.length() > 5)
                                                ret.add(term.substring(0, 5));
                                        return ret;
                                }, "*inputTerms").out("*terms")
                                .macro(termsSearchQuery(
                                                "$$profileTerms",
                                                (String paramsVar) -> Block
                                                                .invokeQuery("bestProfileTermSearch", "*terms",
                                                                                "*limit")
                                                                .out(paramsVar),
                                                (String termVar, String keyVar) -> Block.hashPartition(termVar)
                                                                .each(Ops.IDENTITY, termVar).out(keyVar),
