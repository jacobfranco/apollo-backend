...
public class Search implements RamaModule {
    ...
    private Block finishProfileTermIndexing(KeyToFixedItemsPStateGroup profileTerms, String nameVar,
        ...
        return Block.each((String username, String displayName) -> {
            ...
            return ret;
                ...
                .each((PersistentHashSet termSet, String term, String username) -> new ProfileSearchRecord(
                        (Set) termSet.disjoin(term).disjoin(username.toLowerCase()), username),
        ...
        return Block.each(Ops.SIZE, "*terms").out("*numTerms")
                .loopWithVars(LoopVars.var("*i", 0)
                        ...
                        Block.each(Ops.GET, "*terms", "*i").out("*term")
                                ...
                                .localSelect(pstateVar, Path.key("*queryKey").sortedMapRangeFrom(0, pageAmount))
                                .out("*infoMap")
                                .each(Search::searchForMatches, new Constant(itemTermMatches), "*infoMap", "*term",
                                ...
                                .ifTrue(new Expr(Ops.EQUAL, "*i", new Expr(Ops.DEC, "*numTerms")),
                                        ...
                                        Block.continueLoop(new Expr(Ops.INC, "*i"), "*nextBest")))
                .out("*info")
                .originPartition();
    }
    ...
    private Block termsSearchQuery(String pstateVar, RamaFunction1<String, Block> bestTermSearchMacro,
        ...
        return Block.ifTrue(new Expr(Ops.IS_NULL, "*startParams"),
                ...
                .loopWithVars(LoopVars.var("*id", "*startId"),
                        Block.ifTrue(
                                new Expr(Ops.OR,
                                        new Expr(Ops.GREATER_THAN_OR_EQUAL, new Expr(Ops.SIZE, "*matchList"), "*limit"),
                                ...
                                Block.localSelect(pstateVar, Path.key("*queryKey")
                                        ...
                                        .each(Search::searchForMatches, new Constant(itemTermMatches), "*infoMap",
                                                "*bestTerm", "*terms", pageAmount, "*iterLimit")
                                        .out("*m")
                                        .each(Ops.GET, "*m", "nextId").out("*nextId")
                                        .each(Ops.GET, "*m", "matchList").out("*newMatchList")
                                        .each((List ret, List newMatchList) -> ret.addAll(newMatchList), "*matchList",
                                                "*newMatchList")
    ...
    private static PersistentVector removeId(List<List<Long>> v, long id) {
        return PersistentVector
                .create(v.stream().filter((List<Long> tuple) -> tuple.get(0) != id).collect(Collectors.toList()));
    }
    ...
    public void define(Setup setup, Topologies topologies) {
        ...
        KeyToFixedItemsPStateGroup statusTerms = new KeyToFixedItemsPStateGroup("$$statusTerms", 1000, String.class,
                StatusSearchRecord.class);
        statusTerms.declarePStates(search);

        // key is term or prefix
        KeyToFixedItemsPStateGroup profileTerms = new KeyToFixedItemsPStateGroup("$$profileTerms", 10000, String.class,
                ProfileSearchRecord.class);
        profileTerms.declarePStates(search);

        // hashtag prefix -> full hashtags
        KeyToUniqueFixedItemsPStateGroup hashtagPrefixes = new KeyToUniqueFixedItemsPStateGroup("$$hashtagPrefixes",
        ...
        search.source("*statusWithIdDepot").out("*microbatch")
                ...
                .batchBlock(
                        Block.subBatch(filterStatusWithIdSubBatch("*microbatch")).out("*statusWithIds")
                                .each((List<StatusWithId> swids) -> swids.stream().map(swid -> swid.status.authorId)
                                        .distinct().collect(Collectors.toList()), "*statusWithIds")
                                .out("*allAccountIds")
        ...
        search.source("*accountEditDepot").out("*microbatch")
                ...
                // index display name
                .each((EditAccount e) -> e.getEdits().stream().filter(EditAccountField::isSetDisplayName).findFirst()
                        .map(TUnion::getFieldValue).orElse(null),
                        "*editAccount")
                .out("*editDisplayName")
                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*editDisplayName"),
                        Block.atomicBlock(
                                Block.macro(finishProfileTermIndexing(profileTerms, "*name", "*editDisplayName"))))
                // update directories
                .each((EditAccount e) -> e.getEdits().stream().filter(EditAccountField::isSetDiscoverable).findFirst()
                        .map(TUnion::getFieldValue).orElse(null),
                        "*editAccount")
        ...
        topologies.query("profileTermsSearch", "*inputTerms", "*startParams", "*limit").out("*info")
                ...
                .macro(termsSearchQuery(
                        "$$profileTerms",
                        (String paramsVar) -> Block.invokeQuery("bestProfileTermSearch", "*terms", "*limit")
                                .out(paramsVar),
                        (String termVar, String keyVar) -> Block.hashPartition(termVar)
                                .each(Ops.IDENTITY, termVar).out(keyVar),
                        Search::numProfileTermMatches));
        ...
        topologies.query("hashtagSearch", "*str", "*startParams", "*limit").out("*info")
                ...
                .macro(termsSearchQuery(
                        ...
                        (String termVar, String keyVar) -> Block
                                .each((String str) -> str.substring(0, Math.min(5, str.length())), termVar).out(keyVar)
