...
public class TrendsAndHashtags implements RamaModule {
                        ...
                        CompoundAgg.map("*pointStatusId",
                                CompoundAgg.list(Agg.last("*pointAuthorId")
                                        ...
                                        Agg.accumulator(new TrendsAgg(), "*authorId", "*timestamp")
                                                .captureNewValInto("*history"))))
                .each((RamaFunction2<Long, Long, StatusPointer>) StatusPointer::new, "*pointAuthorId", "*pointStatusId")
                .out("*pointer")
                .each(TrendsAndHashtags::historyStats, "*history").out("*stats");
    ...
    private Block computeReviewedTrendsBlock(String trendsPState, String reviewedPState, String reviewedTrendsPState) {
        return Block.batchBlock(
                Block.localSelect(trendsPState, Path.stay()).out("*trends")
                        ...
                        .each((Map res) -> res == null ? new ArrayList() : new ArrayList(new TreeMap(res).values()),
                                "*res")
                        .out("*reviewedTrends")
    ...
    public void define(Setup setup, Topologies topologies) {
        ...
        KeyToUniqueFixedItemsPStateGroup hashtagToStatusPointers = new KeyToUniqueFixedItemsPStateGroup(
                "$$hashtagToStatusPointers", Core.DEFAULT_TIMELINE_MAX_AMOUNT, String.class, StatusPointer.class);
        ...
        core.source("*statusWithIdDepot").out("*microbatch")
                        ...
                        Block.explodeMicrobatch("*microbatch").out("*data")
                                ...
                                .ifTrue(new Expr(Ops.IS_INSTANCE_OF, BoostStatusContent.class, "*content"),
                                        Block.each(Ops.IDENTITY, null).out("*hashtags")
                                                ...
                                                .each((BoostStatusContent c) -> Arrays.asList(c.boosted.authorId,
                                                        c.boosted.statusId), "*content")
                                                .out("*tuplePoint"),
                                        Block.macro(ApolloHelpers.extractFields("*content", "*visibility", "*text"))
                                                ...
                                                .ifTrue(new Expr(Ops.IS_INSTANCE_OF, ReplyStatusContent.class,
                                                        ...
                                                        Block.each(
                                                                (ReplyStatusContent c) -> Arrays
                                                                        .asList(c.parent.authorId, c.parent.statusId),
                                                                "*content").out("*tuplePoint"),
                                                ...
                                                .each((Set<String> links) -> {
                                                    ...
                                                    for (String url : links) {
                                                        try {
                                                            ret.add(ApolloHelpers.normalizeURL(url));
                ...
                .hook("dataRoot")
                .batchBlock(
                        Block.subBatch(accountHashtagTimelineSubbatch("$$hashtags", "$$accountIdToHashtagActivity"))
                                .out("*authorId", "*hashtag")
                                .hashPartition("*authorId")
                                .compoundAgg("$$accountIdToRecentHashtags",
                                        CompoundAgg.map("*authorId", Agg.list("*hashtag")))
                                ...
                                .localTransform("$$accountIdToRecentHashtags",
                                        Path.key("*authorId").term(TrendsAndHashtags::consolidateRecentHashtags,
                                                accountRecentHashtagsAmount))
                                .localSelect("$$accountIdToRecentHashtags", Path.key("*authorId")).out("*after"))

                .hook("dataRoot")
                .macro(trendsRankingMacro(TrendsAndHashtags::itemHistorySubBatch, "$$hashtags", "$$hashtagStats",
                        "$$hashtagTrends"))

                .hook("dataRoot")
                .macro(trendsRankingMacro(TrendsAndHashtags::itemHistorySubBatch, "$$links", "$$linkStats",
                        "$$linkTrends"))

                .hook("dataRoot")
                .macro(trendsRankingMacro(TrendsAndHashtags::statusHistorySubBatch, "$$statusPoints", "$$statusStats",
                        "$$statusTrends"));
        ...
        core.source("*likeStatusDepot").out("*microbatch")
                .batchBlock(
                        Block.explodeMicrobatch("*microbatch").out("*data")
                                ...
                                .materialize("*pointAuthorId", "*pointStatusId", "*accountId", "*timestamp")
                                .out("$$statusLikePoints"))
                .macro(trendsRankingMacro(TrendsAndHashtags::statusHistorySubBatch, "$$statusLikePoints",
        ...
        KeyToLinkedEntitySetPStateGroup featurerToHashtags = new KeyToLinkedEntitySetPStateGroup("$$featurerToHashtags",
        ...
        feature.source("*featureHashtagDepot", StreamSourceOptions.retryAllAfter()).out("*data")
                .subSource("*data",
                        SubSource.create(FeatureHashtag.class)
                                ...
                                .localSelect("$$featurerToHashtags", Path.key("*accountId").view(Ops.SIZE))
                                .out("*currSize")
        ...
        topologies.query("getFeaturedHashtags", "*accountId").out("*results")
                ...
                .each((RamaFunction3<String, Integer, Long, FeaturedHashtagInfo>) FeaturedHashtagInfo::new, "*hashtag",
