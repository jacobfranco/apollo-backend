...
import com.apollo.backend.ApolloHelpers.ExtractItem;
...
public class TrendsAndHashtags implements RamaModule {
        ...
        public static class TrendsAgg implements RamaAccumulatorAgg2<TreeMap, Long, Long> {
                ...
                public TreeMap accumulate(TreeMap m, Long authorId, Long timestampMillis) {
                        ...
                        List tuple;
                        if (m.containsKey(dayBucket))
                                tuple = (List) m.get(dayBucket);
                        else
                                tuple = Arrays.asList(0, new RHyperLogLog(9));
        ...
        public static List decayTrends(List<List> trends) {
                ...
                for (List tuple : trends == null ? new ArrayList<List>() : trends) {
                        ...
                        for (Map.Entry<Long, Long> e : scores.entrySet()) {
                                if (currDayBucket <= e.getKey() + 1)
                                        newScores.put(e.getKey(), e.getValue());
                        }
        ...
        private static Map historyStats(TreeMap history) {
                ...
                if (history.containsKey(bucket - 1))
                        ret.put(bucket - 1, ((RHyperLogLog) ((List) history.get(bucket - 1)).get(1)).hll.cardinality());
                return ret;
        }

        private static SubBatch itemHistorySubBatch(String sourcePStateVar, String targetPStateVar) {
                Block b = Block.explodeMaterialized(sourcePStateVar)
                                .out("*authorId", "*statusId", "*timestamp", "*items")
                                .each(Ops.EXPLODE, "*items").out("*item")
                                .hashPartition("*item")
                                .compoundAgg(targetPStateVar,
                                                CompoundAgg.map("*item",
                                                                CompoundAgg.list(Agg.accumulator(new TrendsAgg(),
                                                                                "*authorId", "*timestamp")
        ...
        private static SubBatch statusHistorySubBatch(String sourcePStateVar, String targetPStateVar) {
                Block b = Block.explodeMaterialized(sourcePStateVar)
                                .out("*pointAuthorId", "*pointStatusId", "*authorId", "*timestamp")
                                // partition the status by its author for consistency in how other status
                                // PStates are partitioned
                                ...
                                .compoundAgg(targetPStateVar,
                                                CompoundAgg.map("*pointStatusId",
                                                                CompoundAgg.list(Agg.last("*pointAuthorId")
                                                                                ...
                                                                                Agg.accumulator(new TrendsAgg(),
                                                                                                "*authorId",
                                                                                                "*timestamp")
                                                                                                .captureNewValInto(
                                                                                                                "*history"))))
                                .each((RamaFunction2<Long, Long, StatusPointer>) StatusPointer::new, "*pointAuthorId",
                                                "*pointStatusId")
                                .out("*pointer")
                                .each(TrendsAndHashtags::historyStats, "*history").out("*stats");
        ...
        private Block trendsRankingMacro(RamaFunction2<String, String, SubBatch> subbatchFn, String sourcePState,
                        String statsPState, String trendsPState) {
                ...
                return Block.batchBlock(
                                Block.subBatch(subbatchFn.invoke(sourcePState, statsPState)).out("*item", "*stats")
                                                ...
                                                .agg(trendsPState, Agg.topMonotonic(this.topAmt, "*tuple")
                                                                ...
                                                                .sortValFunction(
                                                                                TrendsAndHashtags::getTrendTupleScore)));
        }
                ...
                review.source(depotVar, StreamSourceOptions.retryAllAfter()).out("*data")
                                ...
                                .subSource("*data",
                                                SubSource.create(ReviewItem.class)
                                                                .localTransform(pstateVar,
                                                                                Path.key("*item").termVal(true)),
                                                SubSource.create(RemoveReviewItem.class)
                                                                .localTransform(pstateVar,
                                                                                Path.key("*item").termVoid()));
        }

        private Block computeReviewedTrendsBlock(String trendsPState, String reviewedPState,
                        String reviewedTrendsPState) {
                return Block.batchBlock(
                                Block.localSelect(trendsPState, Path.stay()).out("*trends")
                                                ...
                                                .each((Map res) -> res == null ? new ArrayList()
                                                                : new ArrayList(new TreeMap(res).values()),
                                                                "*res")
                                                .out("*reviewedTrends")
        ...
        private void batchItemQuery(Topologies topologies, String name, String statsPState, String reviewedPState) {
                topologies.query(name, "*items").out("*ret")
                                ...
                                .each((List info, Boolean reviewed, Long timestampMillis) -> {
                                        if (info == null)
                                                return new ItemStats(new HashMap(), 0, -1, reviewed);
                                        long dayBucket = toDayBucket(timestampMillis);
                                        ...
                                        for (Map.Entry<Long, List> entry : history.entrySet()) {
                                                ...
                                                dayBuckets.put(entry.getKey(), new DayBucket((int) tuple.get(0),
                                                                (int) ((RHyperLogLog) tuple.get(1)).hll.cardinality()));
                                        }
                                        return new ItemStats(dayBuckets, (long) info.get(STATUS_COUNT_INDEX),
                                                        (long) info.get(LATEST_STATUS_TIMESTAMP_INDEX), reviewed);
        ...
        private SubBatch accountHashtagTimelineSubbatch(String hashtagPState, String accountIdToHashtagActivityPState) {
                Block b = Block.explodeMaterialized("$$hashtags")
                                .out("*authorId", "*statusId", "*timestamp", "*hashtags")
        ...
        private SubBatch accountSpaceTimelineSubbatch(String spacePState, String accountIdToSpaceActivityPState) {
                Block b = Block.explodeMaterialized("$$spaces")
                                .out("*authorId", "*statusId", "*timestamp", "*spaces")
                                .each(Ops.EXPLODE, "*spaces").out("*space")
                                .hashPartition("*authorId")
                                .compoundAgg(accountIdToSpaceActivityPState,
                                                CompoundAgg.map("*authorId",
                                                                CompoundAgg.map("*space",
                                                                                CompoundAgg.map("timeline",
                                                                                                Agg.set("*statusId"),
                                                                                                "lastStatusMillis",
                                                                                                Agg.max("*timestamp")
                                                                                                                .captureNewValInto(
                                                                                                                                "*ts")))));
                return new SubBatch(b, "*authorId", "*space");
        }
        ...
        private static PersistentVector consolidateRecentSpaces(PersistentVector v, int topAmt) {
                List keep = new ArrayList();
                Set seen = new HashSet();
                ISeq seq = v.rseq();
                while (seq != null && keep.size() < topAmt) {
        ...
        public void define(Setup setup, Topologies topologies) {
                ...
                KeyToUniqueFixedItemsPStateGroup hashtagToStatusPointers = new KeyToUniqueFixedItemsPStateGroup(
                                "$$hashtagToStatusPointers", Core.DEFAULT_TIMELINE_MAX_AMOUNT, String.class,
                                StatusPointer.class);
                hashtagToStatusPointers.declarePStates(core);

                KeyToUniqueFixedItemsPStateGroup spaceToStatusPointers = new KeyToUniqueFixedItemsPStateGroup(
                                "$$spaceToStatusPointers", Core.DEFAULT_TIMELINE_MAX_AMOUNT, String.class,
                                StatusPointer.class);
                spaceToStatusPointers.declarePStates(core);

                // index 0 is history, index 1 is statusCount, and index 2 is latest status
                // timestamp
                core.pstate("$$hashtagStats", PState.mapSchema(String.class, PState.listSchema(Object.class)));
                core.pstate("$$spaceStats", PState.mapSchema(String.class, PState.listSchema(Object.class)));
                ...
                core.pstate("$$spaceTrends", List.class).global();
                ...
                core.pstate("$$reviewedSpaceTrends", List.class).global();
                ...
                core.pstate("$$accountIdToHashtagActivity",
                                PState.mapSchema(Long.class,
                                                PState.mapSchema(String.class,
                                                                ...
                                                                .subindexed()));
                core.pstate("$$accountIdToSpaceActivity",
                                PState.mapSchema(Long.class,
                                                PState.mapSchema(String.class,
                                                                PState.fixedKeysSchema("timeline",
                                                                                PState.setSchema(Long.class)
                ...
                core.pstate("$$accountIdToRecentSpaces", PState.mapSchema(Long.class, List.class));

                core.source("*statusWithIdDepot").out("*microbatch")
                                .batchBlock(
                                                Block.explodeMicrobatch("*microbatch").out("*data")
                                                                .keepTrue(new Expr(Ops.OR, new Expr(Ops.IS_INSTANCE_OF,
                                                                                EditStatus.class, "*data"),
                                                                ...
                                                                .ifTrue(new Expr(Ops.IS_INSTANCE_OF,
                                                                                ...
                                                                                Block.each(Ops.IDENTITY, null)
                                                                                                ...
                                                                                                .each(Ops.IDENTITY,
                                                                                                                null)
                                                                                                .out("*links")
                                                                                                .each((BoostStatusContent c) -> Arrays
                                                                                                                .asList(c.boosted.authorId,
                                                                                                                                c.boosted.statusId),
                                                                                                                "*content")
                                                                                                .out("*tuplePoint"),
                                                                                Block.macro(ApolloHelpers.extractFields(
                                                                                                ...
                                                                                                .ifTrue(new Expr(
                                                                                                                ...
                                                                                                                "*content"),
                                                                                                                Block.each(
                                                                                                                                (ReplyStatusContent c) -> Arrays
                                                                                                                                                .asList(c.parent.authorId,
                                                                                                                                                                c.parent.statusId),
                                                                                                                                "*content")
                                                                                                                                .out("*tuplePoint"),
                                                                ...
                                                                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*tuplePoint"),
                                                                                Block.each(Ops.EXPAND, "*tuplePoint")
                                                                                                .out("*pointAuthorId",
                                                                                                                "*pointStatusId")
                                                                                                .materialize("*pointAuthorId",
                                                                                                                "*pointStatusId",
                                                                                                                "*authorId",
                                                                                                                "*timestamp")
                                                                                                .out("$$statusPoints")))
                                .anchor("dataRoot")
                                .batchBlock(
                                                Block.explodeMaterialized("$$hashtags")
                                                                .out("*authorId", "*statusId", "*timestamp",
                                                                                "*hashtags")
                                                                .each(Ops.EXPLODE, "*hashtags").out("*hashtag")
                                                                ...
                                                                .each((RamaFunction2<Long, Long, StatusPointer>) StatusPointer::new,
                                                                                ...
                                                                                "*statusId")
                                                                .out("*statusPointer")
                                                                .macro(hashtagToStatusPointers.addItem("*hashtag",
                                                                                "*statusPointer")))
                                .hook("dataRoot")
                                .batchBlock(
                                                Block.explodeMaterialized("$$spaces")
                                                                .out("*authorId", "*statusId", "*timestamp",
                                                                                "*spaces")
                                                                .each(Ops.EXPLODE, "*spaces").out("*space")
                                                                .hashPartition("*space")
                                                                .each((RamaFunction2<Long, Long, StatusPointer>) StatusPointer::new,
                                                                                ...
                                                                                "*statusId")
                                                                .out("*statusPointer")
                                                                .macro(spaceToStatusPointers.addItem("*space",
                                                                                "*statusPointer")))

                                .hook("dataRoot")
                                .batchBlock(
                                                Block.subBatch(accountHashtagTimelineSubbatch("$$hashtags",
                                                                "$$accountIdToHashtagActivity"))
                                                                .out("*authorId", "*hashtag")
                                                                .hashPartition("*authorId")
                                                                .compoundAgg("$$accountIdToRecentHashtags",
                                                                                CompoundAgg.map("*authorId",
                                                                                                Agg.list("*hashtag")))
                                                                .agg(Agg.list("*authorId")).out("*authors")
                                                                .each(Ops.EXPLODE, "*authors").out("*authorId")
                                                                .localSelect("$$accountIdToRecentHashtags",
                                                                                Path.key("*authorId"))
                                                                .out("*before")
                                                                .localTransform("$$accountIdToRecentHashtags",
                                                                                Path.key("*authorId").term(
                                                                                                TrendsAndHashtags::consolidateRecentHashtags,
                                                                                                accountRecentHashtagsAmount))
                                                                .localSelect("$$accountIdToRecentHashtags",
                                                                                Path.key("*authorId"))
                                                                .out("*after"))
                                .hook("dataRoot")
                                .batchBlock(
                                                Block.subBatch(accountSpaceTimelineSubbatch("$$spaces",
                                                                "$$accountIdToSpaceActivity"))
                                                                .out("*authorId", "*space")
                                                                .hashPartition("*authorId")
                                                                .compoundAgg("$$accountIdToRecentSpaces",
                                                                                CompoundAgg.map("*authorId",
                                                                                                Agg.list("*space")))
                                                                .agg(Agg.list("*authorId")).out("*authors")
                                                                .each(Ops.EXPLODE, "*authors").out("*authorId")
                                                                .localSelect("$$accountIdToRecentSpaces",
                                                                                Path.key("*authorId"))
                                                                .out("*before")
                                                                .localTransform("$$accountIdToRecentSpaces",
                                                                                Path.key("*authorId").term(
                                                                                                TrendsAndHashtags::consolidateRecentSpaces,
                                                                                                accountRecentSpacesAmount))
                                                                .localSelect("$$accountIdToRecentSpaces",
                                                                                Path.key("*authorId"))
                                                                .out("*after"))

                                .hook("dataRoot")
                                .macro(trendsRankingMacro(TrendsAndHashtags::itemHistorySubBatch, "$$hashtags",
                                                "$$hashtagStats",
                                                "$$hashtagTrends"))
                                ...
                                .hook("dataRoot")
                                .macro(trendsRankingMacro(TrendsAndHashtags::itemHistorySubBatch, "$$links",
                                                "$$linkStats",
                                                "$$linkTrends"))

                                .hook("dataRoot")
                                .macro(trendsRankingMacro(TrendsAndHashtags::statusHistorySubBatch, "$$statusPoints",
                                                "$$statusStats",
                                                "$$statusTrends"));

                core.source("*likeStatusDepot").out("*microbatch")
                                .batchBlock(
                                                Block.explodeMicrobatch("*microbatch").out("*data")
                                                                .keepTrue(new Expr(Ops.IS_INSTANCE_OF, LikeStatus.class,
                                                                                "*data"))
                                                                .macro(extractFields("*data", "*accountId", "*target",
                                                                                "*timestamp"))
                                                                .each(StatusPointer::getAuthorId, "*target")
                                                                .out("*pointAuthorId")
                                                                .each(StatusPointer::getStatusId, "*target")
                                                                .out("*pointStatusId")
                                                                .materialize("*pointAuthorId", "*pointStatusId",
                                                                                "*accountId", "*timestamp")
                                                                .out("$$statusLikePoints"))
                                .macro(trendsRankingMacro(TrendsAndHashtags::statusHistorySubBatch,
                                                "$$statusLikePoints",
                                                "$$statusStats", "$$statusTrends"));
                ...
                core.source("*reviewTick").out("*microbatch")
                                .explodeMicrobatch("*microbatch")
                                .macro(computeReviewedTrendsBlock("$$hashtagTrends", "$$reviewedHashtags",
                                                "$$reviewedHashtagTrends"))
                                .macro(computeReviewedTrendsBlock("$$spaceTrends", "$$reviewedSpaces",
                                                "$$reviewedSpaceTrends"))
                                .macro(computeReviewedTrendsBlock("$$linkTrends", "$$reviewedLinks",
                                                "$$reviewedLinkTrends"));
                ...
                topologies.query("getHashtagTimeline", "*hashtag", "*requestAccountId", "*firstTimelineIndex", "*limit")
                                .out("*results")
                                .hashPartition("*hashtag")
                ...
                topologies.query("getSpaceTimeline", "*space", "*requestAccountId", "*firstTimelineIndex", "*limit")
                                .out("*results")
                                .hashPartition("*space")
                                .each((Integer limit) -> SortedRangeFromOptions.excludeStart().maxAmt(limit), "*limit")
                                .out("*sortedOptions")
                                .localSelect("$$spaceToStatusPointers",
                                                Path.subselect(Path.key("*space")
                                                                .sortedMapRangeFrom("*firstTimelineIndex",
                                                                                "*sortedOptions")
                                                                .mapVals()))
                                .out("*statusPointers")
