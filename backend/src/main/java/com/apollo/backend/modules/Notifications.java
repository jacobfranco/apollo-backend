...
public class Notifications implements RamaModule {
    ...
    private static void extractUserContentNotifications(long authorId, long statusId, long timestamp, String text,
            PollContent pc, OutputCollector collector) {
        if (pc != null)
            collector.emitStream("polls", pc.getExpirationMillis(), new StatusPointer(authorId, statusId));
        for (String mention : Token.filterMentions(Token.parseTokens(text))) {
            collector.emitStream("username", mention,
                    new Notification(NotificationContent.mention(new StatusPointer(authorId, statusId)), timestamp));
        }
    }
    ...
    public Block keepNonSuppressedMacro(Object targetId, Object sourceAccountId, Object authorId, Object statusId) {
        ...
        return Block
                .select("$$accountIdToSuppressions",
                        Relationships.isBlockedOrMutedPath(targetId, sourceAccountId, true))
                .out(isSuppressedVar)
                ...
                .select("$$muterToStatusIds", Path.key(targetId).view(Ops.CONTAINS, convoIdVar))
                .out(isStatusSuppressedVar)
                .keepTrue(new Expr(Ops.AND, new Expr(Ops.NOT, isSuppressedVar),
                        new Expr(Ops.NOT, isStatusSuppressedVar)));
    }
    ...
    public void define(Setup setup, Topologies topologies) {
        ...
        setup.clusterPState("$$followeeToNotifiedFollowerIds", Relationships.class.getName(),
                "$$followeeToNotifiedFollowerIds");

        MicrobatchTopology notifications = topologies.microbatch("notifications");
        KeyToFixedItemsPStateGroup accountIdToNotificationsTimeline = new KeyToFixedItemsPStateGroup(
                "$$accountIdToNotificationsTimeline", 800, Long.class, Notification.class);
        ...
        notifications.pstate("$$statusWithIdToFollowerFanout",
                PState.mapSchema(StatusWithId.class, FollowerFanout.class));
        ...
        notifications.source("*statusWithIdDepot").out("*microbatch")
                ...
                .macro(pollScheduler.handleExpirations("*pointer", "*currentTimeMillis",
                        Block.macro(extractFields("*pointer", "*authorId", "*statusId"))
                                .select("$$accountIdToStatuses", Path.key("*authorId", "*statusId").first())
                                .out("*status")
                                .keepTrue(new Expr((Status s, Long currentTimeMillis) -> {
                                    ...
                                    PollContent pc = ApolloHelpers.extractPollContent(s);
                                    return pc != null && pc.getExpirationMillis() <= currentTimeMillis;
                                }, "*status", "*currentTimeMillis"))
                                .each((Long authorId, Long statusId, Long currentTimeMillis) -> new Notification(
                                        NotificationContent.pollComplete(new StatusPointer(authorId, statusId)),
                                        currentTimeMillis),
                                        "*authorId", "*statusId", "*currentTimeMillis")
                                .out("*notification")
                                .anchor("PollFanoutRoot")))

                .hook("PollFanoutRoot")
                // This fans out poll expiration to the creator of the poll even if they didn't
                // vote in it.
                        ...
                        Block.yieldIfOvertime()
                                .localSelect("$$pollVotes",
                                        Path.key("*statusId", "allVoters")
                                                .sortedMapRangeFrom("*i",
                                                        SortedRangeFromOptions.excludeStart().maxAmt(BULK_FETCH_SIZE))
                                                .subselect(Path.mapKeys()))
                                .out("*targetIds")
                                .atomicBlock(
                                        Block.each(Ops.EXPLODE, "*targetIds").out("*targetId")
                                                .emitLoop("*targetId"))
                                .ifTrue(new Expr(Ops.EQUAL, new Expr(Ops.SIZE, "*targetIds"), BULK_FETCH_SIZE),
                                        Block.continueLoop(new Expr(Ops.LAST, "*targetIds"))))
                .out("*targetId")
                // If author of poll voted on it, don't fan it out here since it's already
                // fanned out
                ...
                // This extracts the different kidns of notifications that can come from
                // statuses.
                .each((Long statusId, Long authorId, Long timestamp, Object content, OutputCollector collector) -> {
                    if (content instanceof ReplyStatusContent) {
                        ...
                        collector.emit(parentAuthorId, new Notification(
                                NotificationContent.mention(new StatusPointer(authorId, statusId)), timestamp));
                        extractUserContentNotifications(authorId, statusId, timestamp,
                                ((ReplyStatusContent) content).getText(),
                                ((ReplyStatusContent) content).getPollContent(), collector);
                        ...
                        collector
                                .emit(parentAuthorId,
                                        new Notification(
                                                NotificationContent.boost(new StatusResponseNotificationContent(
                                                        authorId, new StatusPointer(parentAuthorId, parentStatusId))),
                                                timestamp));
                    } else {
                        extractUserContentNotifications(authorId, statusId, timestamp,
                                ((NormalStatusContent) content).getText(),
                                ((NormalStatusContent) content).getPollContent(), collector);
                    }
                }, "*statusId", "*authorId", "*timestamp", "*content")
                .outStream("username", "Username", "*targetName", "*notification")
                ...
                .keepTrue(new Expr((Object content, Long targetId) -> {
                    if (content instanceof ReplyStatusContent)
                        return ((ReplyStatusContent) content).getParent().authorId != targetId;
                    else
                        return true;
                }, "*content", "*targetId"))
                .anchor("NameMention")
                ...
                .each((Long authorId, Long statusId, Long timestamp) -> new Notification(
                        NotificationContent.boostedUpdate(new StatusPointer(authorId, statusId)), timestamp),
                        "*authorId", "*statusId", "*timestamp")
                .out("*notification")
                .loopWithVars(LoopVars.var("*i", -1),
                        Block.yieldIfOvertime()
                                ...
                                .atomicBlock(
                                        Block.each(Ops.EXPLODE_MAP, "*targetMap").out("*j", "*targetPointer")
                                                .each((StatusPointer sp) -> sp.authorId, "*targetPointer")
                                                .out("*targetId")
                                                .emitLoop("*targetId"))
                ...
                .each((StatusWithId statusWithId) -> {
                    StatusVisibility vis = ApolloHelpers.getStatusVisibility(statusWithId.status);
                    return vis == StatusVisibility.Public || vis == StatusVisibility.Private
                            || vis == StatusVisibility.Unlisted;
                ...
                .loopWithVars(LoopVars.var("*loopNextIndex", "*nextIndex"),
                        Block.localSelect("$$followeeToNotifiedFollowerIds",
                                Path.key("*authorId").sortedSetRangeFrom("*loopNextIndex",
                                        SortedRangeFromOptions.excludeStart().maxAmt(rangeQueryLimit)))
                                .out("*idsBatch")
                                ...
                                .ifTrue(new Expr(Ops.OR, new Expr(Ops.IS_NULL, "*nextUnhandledId"),
                                        new Expr(Ops.GREATER_THAN_OR_EQUAL, new Expr(Ops.SIZE, "*followerIds"),
                                                fanoutLimit)),
                                        Block.emitLoop("*nextUnhandledId"),
                                        Block.continueLoop("*nextUnhandledId")))
                .out("*nextFanoutStart")
                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*nextFanoutStart"),
                        Block.each(
                                (Long authorId, Long nextFollowerId, Status status) -> new FollowerFanout(authorId,
                                        nextFollowerId, FanoutAction.Add, status, -1),
                                "*authorId", "*nextFanoutStart", "*status").out("*followerFanout")
                                .localTransform("$$statusWithIdToFollowerFanout",
                                        Path.key("*statusWithId").termVal("*followerFanout")))
                .macro(extractFields("*status", "*timestamp"))
                .each((StatusWithId statusWithId, Long timestamp) -> {
                    return new Notification(NotificationContent.followeeStatus(
                            new StatusPointer(statusWithId.status.authorId, statusWithId.statusId)), timestamp);
        ...
        notifications.source("*followAndBlockAccountDepot").out("*microbatch")
                ...
                .each((Long requesterId,
                        Long timestamp) -> new Notification(NotificationContent.followRequest(requesterId), timestamp),
                ...
                .hashPartition("*targetId")
                .each((Long accountId, Long timestamp) -> new Notification(NotificationContent.follow(accountId),
                        timestamp),
                        "*accountId", "*timestamp")
                .out("*notification")
                .macro(accountIdToNotificationsTimeline.addItem("*targetId", "*notification"));
