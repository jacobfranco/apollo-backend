...
public class Core implements RamaModule {
    ...
    public static class Timeline {
        ...
        public long startIndexTimelineIndex = Long.MAX_VALUE; // timeline index (always decreasing for every append) of
                                                              // startIndex
            ...
            if (tuples.size() > 0) {
                ...
                for (List tuple : tuples) {
                    if (appendable.size() == numToAdd)
                        break;
                    if (!existingSet.contains(new StatusPointer((Long) tuple.get(1), (Long) tuple.get(2))))
  ...
  private void declareFollowsBloomFiltersTopology(Topologies topologies) {
    ...
    private void declareMicrobatchTopologies(Topologies topologies) {
        ...
        KeyToLinkedEntitySetPStateGroup likerToStatusPointers = new KeyToLinkedEntitySetPStateGroup("$$likerToStatusPointers", Long.class, StatusPointer.class).descending();
        KeyToLinkedEntitySetPStateGroup statusIdToLikers = new KeyToLinkedEntitySetPStateGroup("$$statusIdToLikers", Long.class, Long.class).descending();
        ...
        core.source("*likeStatusDepot").out("*mb")
            ...
            .subSource("*data",
                    SubSource.create(LikeStatus.class)
                             ...
                             .macro(likerToStatusPointers.addToLinkedSet("*accountId", "*target"))
                             .hashPartition("*authorId")
                             .macro(statusIdToLikers.addToLinkedSet("*statusId", "*accountId")),
                    SubSource.create(RemoveLikeStatus.class)
                             .macro(extractFields("*data", "*accountId", "*target"))
                             .macro(extractFields("*target", "*authorId", "*statusId"))
                             .macro(likerToStatusPointers.removeFromLinkedSet("*accountId", "*target"))
                             .hashPartition("*authorId")
                             .macro(statusIdToLikers.removeFromLinkedSet("*statusId", "*accountId")));
                ...
                .ifTrue(new Expr(Ops.OR, new Expr(Ops.IS_NULL, "*currInfo"),
                        ...
                        Block.macro(accountIdGen.genId("*accountId"))
                                .localTransform("$$nameToUser",
                                        Path.key("*name").multiPath(Path.key("accountId").termVal("*accountId"),
    ...
    private void declareStatusTopology(Topologies topologies) {
        ...
        // Declare PStates for account statuses, account timelines, post UUIDs to status IDs mapping
        ...
        KeyToLinkedEntitySetPStateGroup statusIdToReplies = new KeyToLinkedEntitySetPStateGroup("$$statusIdToReplies",
        ...
        KeyToLinkedEntitySetPStateGroup statusIdToBoosters = new KeyToLinkedEntitySetPStateGroup("$$statusIdToBoosters",
                Long.class, StatusPointer.class)
        ...
        stream.pstate("$$pollVotes",
                PState.mapSchema(
                        Long.class, // statusId
                        PState.fixedKeysSchema("allVoters", PState.mapSchema(Long.class, Set.class).subindexed(), // accountId
                                                                                                                  // ->
                                                                                                                  // choicesIndexes
                                "choices",
                                PState.mapSchema(Integer.class, PState.setSchema(Long.class).subindexed())))); // choiceIndex
                                                                                                               // -> set
                                                                                                               // of
                                                                                                               // accountId
        ...
        stream.source("*statusDepot").out("*initialData")
                ...
                .ifTrue(new Expr(Ops.IS_INSTANCE_OF, BoostStatus.class, "*initialData"),
                        Block.each(ApolloHelpers::createAddStatusFromBoost, "*initialData").out("*data"),
                        Block.each(Ops.IDENTITY, "*initialData").out("*data"))
                // Process the data further based on its type.
                .subSource("*data",
                        SubSource.create(AddStatus.class)
                                // Extract and map necessary fields from the status data.
                                .macro(extractFields("*data", "*uuid", "*status"))
                                .macro(extractFields("*status", "*authorId", "*content"))

                                // Attempt to select an existing status ID by UUID; if it exists, use it,
                                ...
                                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*statusIdMaybe"),
                                        ...
                                        Block.macro(statusIdGen.genId("*statusId"))
                                                .localTransform("$$postUUIDToStatusId",
                                                        Path.key("*uuid").termVal("*statusId")))
                                // Transform the account's statuses by appending the current status under the
                                // generated or selected ID.
                                .localTransform("$$accountIdToStatuses",
                                        Path.key("*authorId", "*statusId").beforeElem().termVal("*status"))
                                // Special handling for reply content, including setting conversation IDs and
                                // updating relevant PStates.
                                .ifTrue(new Expr(Ops.IS_INSTANCE_OF, ReplyStatusContent.class, "*content"),
                                        Block.macro(ApolloHelpers.extractFields("*content", "*parent"))
                                                .each((StatusPointer parent) -> parent.authorId, "*parent")
                                                .out("*parentAuthorId")
                                                .each((StatusPointer parent) -> parent.statusId, "*parent")
                                                .out("*parentStatusId")
                                                .hashPartition("*parentAuthorId")
                                                .localSelect("$$statusIdToConvoId",
                                                        Path.key("*parentStatusId").nullToVal("*parentStatusId"))
                                                .out("*convoId")
                                                .hashPartition("*authorId")
                                                .localTransform("$$statusIdToConvoId",
                                                        Path.key("*statusId").termVal("*convoId")))
                                // Ensure updates to $$accountIdToStatuses and $$statusIdToConvoId occur before
                                // fanout happens.
                                .hashPartition("*authorId")
                                // Wrap the status in a StatusWithId object before appending it to a depot.
                                .each((RamaFunction2<Long, Status, StatusWithId>) StatusWithId::new, "*statusId",
                                        "*status")
                                .out("*statusWithId")
                                .depotPartitionAppend("*statusWithIdDepot", "*statusWithId")

                                // Exclude direct messages from the account timeline.
                                .ifTrue(new Expr(Ops.NOT_EQUAL, new Expr(ApolloHelpers::getStatusVisibility, "*status"),
                                        StatusVisibility.Direct),
                                        Block.localTransform("$$accountIdToAccountTimeline",
                                                Path.key("*authorId").voidSetElem().termVal("*statusId")))
                                // Process attachments, adding status ID to a set for query purposes if
                                // attachments are present.
                                .ifTrue(new Expr(Ops.OR,
                                        new Expr(Ops.IS_INSTANCE_OF, NormalStatusContent.class, "*content"),
                                        new Expr(Ops.IS_INSTANCE_OF, ReplyStatusContent.class, "*content")),
                                        Block.macro(extractFields("*content", "*attachments"))
                                                .ifTrue(new Expr(Ops.AND, new Expr(Ops.IS_NOT_NULL, "*attachments"),
                                                        new Expr(Ops.GREATER_THAN, new Expr(Ops.SIZE, "*attachments"),
                                                                0)),
                                                        // Add status ID to a set to enable querying all statuses with
                                                        // attachments.
                                                        Block.localTransform("$$accountIdToAttachmentStatusIds",
                                                                Path.key("*authorId").voidSetElem()
                                                                        .termVal("*statusId"))))
                                // Process reply statuses by creating a link between the reply and its parent
                                // status.
                                .ifTrue(new Expr(Ops.IS_INSTANCE_OF, ReplyStatusContent.class, "*content"),
                                        Block.each((RamaFunction2<Long, Long, StatusPointer>) StatusPointer::new,
                                                "*authorId", "*statusId").out("*statusPointer")
                                                .macro(ApolloHelpers.extractFields("*content", "*parent"))
                                                .each((StatusPointer parent) -> parent.authorId, "*parent")
                                                .out("*parentAuthorId")
                                                .each((StatusPointer parent) -> parent.statusId, "*parent")
                                                .out("*parentStatusId")
                                                .hashPartition("*parentAuthorId")
                                                // Add the reply status to a linked set organized by parent status ID.
                                                .macro(statusIdToReplies.addToLinkedSet("*parentStatusId",
                                                        "*statusPointer")))
                                // Handle boosted statuses by linking boosters to the original status.
                                .ifTrue(new Expr(Ops.IS_INSTANCE_OF, BoostStatusContent.class, "*content"),
                                        Block.macro(extractFields("*content", "*boosted"))
                                                .each((StatusPointer boosted) -> boosted.authorId, "*boosted")
                                                .out("*boostedAuthorId")
                                                .each((StatusPointer boosted) -> boosted.statusId, "*boosted")
                                                .out("*boostedStatusId")
                                                .hashPartition("*boostedAuthorId")
                                                .each((RamaFunction2<Long, Long, StatusPointer>) StatusPointer::new,
                                                        "*authorId", "*statusId")
                                                .out("*sp")
                                                // Add the booster to a linked set organized by the original status ID.
                                                .macro(statusIdToBoosters.addToLinkedSet("*boostedStatusId", "*sp"))),

                        // Begin processing for editing a status post, targeting the 'EditStatus' type
                        // of data. TODO: Possibly remove this functionality or make it for verified
                        // users
                        SubSource.create(EditStatus.class)
                                // Extract 'statusId' and 'status' fields from the data to be processed.
                                .macro(extractFields("*data", "*statusId", "*status"))
                                // Extract 'authorId' from the status to identify the post's owner.
                                .macro(extractFields("*status", "*authorId"))
                                // Attempt to select the previous version of the status post using the authorId
                                // and statusId.
                                // This operation also applies a limit to the number of edits allowed
                                // (maxEditCount).
                                .localSelect("$$accountIdToStatuses", Path.key("*authorId", "*statusId")
                                        .view((List l, Integer max) -> {
                                            // If there are too many edits or none, return null; otherwise, return the
                                            // first post.
                                ...
                                .out("*prevStatus")
                                // Update the PState for account statuses by inserting the new status content,
                                // ensuring it does not exceed the maximum allowed edits.
                                .localTransform("$$accountIdToStatuses", Path.key("*authorId").must("*statusId")
                                        ...
                                        .beforeElem()
                                        .termVal("*status"))
                                // Check if the status edit involves changes to a poll. If so, and if a new poll
                                ...
                                // Append the edited status data to a dedicated depot for processed status
                                // updates.
                        ...
                        SubSource.create(RemoveStatus.class)
                                // Extract necessary identifiers from the data: accountId and statusId.
                                .macro(extractFields("*data", "*accountId", "*statusId"))
                                // Invoke a macro to handle the removal logic for the specified status post.
                        ...
                        SubSource.create(RemoveBoostStatus.class)
                                // Extract accountId and target (the original status being boosted) from the
                                // data.
                                .macro(extractFields("*data", "*accountId", "*target"))
                                // Further extract the authorId and statusId of the original status from the
                                // target.
                                .macro(extractFields("*target", "*authorId", "*statusId"))
                                // Partition the stream by the authorId of the original status to ensure
                                // consistency in distributed processing.
                                .hashPartition("*authorId")
                                // Select the identifier for the boost relationship based on the statusId and
                                // accountId.
                                // This operation ensures that only the correct boost relationship is targeted
                                // for removal.
                                .localSelect("$$statusIdToBoosters", Path.key("*statusId").must("*accountId"))
                                .out("*id")
                                // Select the specific boost relationship using the previously obtained
                                // identifier.
                                .localSelect("$$statusIdToBoostersById", Path.key("*statusId", "*id")).out("*sp")
                                // Extract the boostStatusId from the selected boost relationship.
                                .each((StatusPointer sp) -> sp.statusId, "*sp").out("*boostStatusId")
                                // Invoke a macro to handle the removal logic for the boosted status post.
                                .macro(removeStatusMacro("*accountId", "*boostStatusId"))
                                // Re-partition the stream by authorId to maintain consistency.
                                .hashPartition("*authorId")
                                // Remove the selected boost relationship from the linked set of boosters by
                                // entityId.
        ...
        stream.source("*statusAttachmentWithIdDepot").out("*data")
                .subSource("*data",
                        ...
                        SubSource.create(AttachmentWithId.class)
                                // Extract 'uuid' (unique identifier for the attachment) and the 'attachment'
                                // itself.
                                .macro(extractFields("*data", "*uuid", "*attachment"))
                                // Update the PState mapping UUIDs to attachments, effectively associating the
        ...
        stream.source("*pollVoteDepot").out("*data")
                ...
                .localTransform("$$pollVotes", Path.key("*statusId")
                        ...
                        .multiPath(Path.key("allVoters").key("*accountId").termVal("*choices"),
                                Path.key("choices")
                                        // Dynamic path building based on the set of choices. For each choice in the
                                        // set,
                                        // a path is created, and the accountId is added to the set of voters for that
                                        // choice.
                                        .pathBuilder((Set choices) -> {
                                            Path.Impl ret = Path.stop();
                                            for (Object c : choices)
                                                ret = Path.multiPath(ret, Path.key(c));
                                            return ret;
                                        }, "*choices")
                                        .voidSetElem()
                                        .termVal("*accountId")));
        ...
        stream.source("*scheduledStatusDepot").out("*data")
                .subSource("*data",
                        ...
                        SubSource.create(AddScheduledStatus.class)
                                // Extract essential fields: uuid, status content, and scheduled publish time.
                                .macro(extractFields("*data", "*uuid", "*status", "*publishMillis"))
                                .macro(extractFields("*status", "*authorId"))
                                // Attempt to select an existing status ID by UUID; if not found, generate a new
                                // one.
                                .localSelect("$$postUUIDToStatusId", Path.key("*uuid")).out("*statusIdMaybe")
                                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*statusIdMaybe"),
                                        ...
                                        Block.macro(statusIdGen.genId("*id"))
                                                .localTransform("$$postUUIDToStatusId",
                                                        Path.key("*uuid").termVal("*id")))
                                // Transform the PState for scheduled statuses with new or updated information.
                                ...
                                .macro(scheduledStatuses.scheduleItem("*publishMillis",
                                        new Expr(Ops.TUPLE, "*authorId", "*id", "*uuid"))),

                        // Process editing an existing status.
                        SubSource.create(EditStatus.class)
                                .macro(extractFields("*data", "*statusId", "*status"))
                                .macro(extractFields("*status", "*authorId"))
                                // Update the scheduled status content within the PState.
                                .localTransform("$$accountIdToScheduledStatuses",
                                        Path.key("*authorId").must("*statusId", "status").termVal("*status")),
                        ...
                        SubSource.create(RemoveStatus.class)
                                ...
                                .localTransform("$$accountIdToScheduledStatuses",
                                        Path.key("*accountId", "*statusId").termVoid()),
                        ...
                        SubSource.create(EditScheduledStatusPublishTime.class)
                                ...
                                // Update the scheduled publish time for a specific status.
                                .localTransform("$$accountIdToScheduledStatuses",
                                        Path.key("*accountId").must("*id", "publishMillis").termVal("*publishMillis"))
                                // Select the UUID for the scheduled status to ensure it is correctly identified
                                // in the schedule.
                                .localSelect("$$accountIdToScheduledStatuses",
                                        Path.key("*accountId").must("*id", "uuid"))
                                .out("*uuid")
                                // Re-schedule the status post with the updated publish time.
                                .macro(scheduledStatuses.scheduleItem("*publishMillis",
                                        new Expr(Ops.TUPLE, "*accountId", "*id", "*uuid"))));
        ...
        stream.source("*scheduledStatusTick")
                ...
                .macro(scheduledStatuses.handleExpirations("*tuple", "*currentTimeMillis",
                        // Expand each tuple to access individual fields: accountId, statusId (id), and
                        // uuid.
                        Block.each(Ops.EXPAND, "*tuple").out("*accountId", "*id", "*uuid")
                                // Select the specific scheduled status information from the PState using
                                // accountId and id.
                                .localSelect("$$accountIdToScheduledStatuses", Path.key("*accountId", "*id")).out("*m")
                                // Check if the selected status information is not null, indicating a scheduled
                                // status exists.
                                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*m"),
                                        // Extract the scheduled publish time and status object from the status
                                        // information.
                                        Block.each(Ops.GET, "*m", "publishMillis").out("*publishMillis")
                                                .each(Ops.GET, "*m", "status").out("*status")
                                                // Check if the current system time is greater than or equal to the
                                                // scheduled publish time.
                                                .ifTrue(new Expr(Ops.GREATER_THAN_OR_EQUAL, "*currentTimeMillis",
                                                        "*publishMillis"),
                                                        // Adjust the timestamp of the status to the current time and
                                                        // handle any associated poll content.
                                                        Block.each((Status status, Long currentTimeMillis,
                                                                String uuid) -> {
                                                            long origTimestamp = status.getTimestamp();
                                                            status.setTimestamp(currentTimeMillis);
                                                            PollContent pc = ApolloHelpers.extractPollContent(status);
                                                            if (pc != null)
                                                                pc.setExpirationMillis(pc.getExpirationMillis() + Math
                                                                        .max(0, currentTimeMillis - origTimestamp));
                                                            // Return a new AddStatus action to publish the status.
                                                            return new AddStatus(uuid, status);
                                                        }, "*status", "*currentTimeMillis", "*uuid").out("*addStatus")
                                                                // Append the ready-to-publish status to the statusDepot
                                                                // for actual publication.
                                                                .depotPartitionAppend("*statusDepot", "*addStatus")
                                                                // Remove the scheduled status entry now that it's been
                                                                // processed.
                                                                .localTransform("$$accountIdToScheduledStatuses",
                                                                        Path.key("*accountId", "*id").termVoid())))));
    ...
    private void declareQueries(Topologies topologies) {
        ...
        topologies.query("getAccountTimeline", "*requestAccountId", "*timelineAccountId", "*firstStatusId", "*limit",
                ...
                .localSelect("$$accountIdToStatuses", Path.must("*timelineAccountId", "*statusId").first())
                .out("*status")
                ...
                .each(ApolloHelpers::updateStatusQueryResults, "*statusQueryResults", "*statusPointers", "*limit",
                        false)
                .out("*results");
        ...
        topologies.query("refreshHomeTimeline", "*accountId").out("*ret")
                ...
                // as specified.
                .select("$$followerToFolloweesById", Path.key("*accountId").sortedMapRangeFrom(0L, 300).mapVals())
        ...
        topologies.query("getHomeTimeline", "*requestAccountId", "*firstStatusPointer", "*limit").out("*ret")
                ...
                .ifTrue(new Expr(Ops.AND, enableHomeTimelineRefresh,
                        new Expr(HomeTimelines::needsRefresh, "*homeTimelines", "*requestAccountId")),
                        // If a refresh is needed, invoke the 'refreshHomeTimeline' query and output the
                        // result.
                        Block.invokeQuery("refreshHomeTimeline", "*requestAccountId")
                                .each(Ops.IDENTITY, true).out("*refreshed"),
                        // If no refresh is needed, proceed without changing the flow but mark as not
                        // refreshed.
                        Block.each(Ops.IDENTITY, false).out("*refreshed"))
                // Read the timeline data from storage, providing necessary parameters.
                .each(HomeTimelines::readTimelineFrom, "*homeTimelines", "*requestAccountId", "*firstStatusPointer",
                        "*limit")
                .out("*statusPointers")
        ...
        topologies.query("getHomeTimelinesUntil", "*tuples", "*limitPerTimeline").out("*ret")
                ...
                .each(HomeTimelines::readTimelineUntil, "*homeTimelines", "*accountId", "*endStatusPointer",
                        "*limitPerTimeline")
                .out("*newPointers")
        ...
        topologies.query("getConversation", "*requestAccountId", "*convoId").out("*conversation")
                ...
                .ifTrue(new Expr(Ops.EQUAL, 0, new Expr(Ops.SIZE, "*keys")),
                        ...
                        // If keys exist, fetch the first status from the conversation timeline as a
                        // starting point for fetching the latest status.
                        Block.localSelect("$$accountIdToConvoIdToConvo",
                                Path.key("*requestAccountId", "*convoId", "timeline").view(Ops.FIRST))
                                .out("*statusIndexAndStatusPointer")
                                .ifTrue(new Expr(Ops.IS_NULL, "*statusIndexAndStatusPointer"),
                                        // If no status index and pointer are found, set the last status of the
                                        // conversation to null.
                                        Block.each(Ops.IDENTITY, null).out("*lastStatus"),
                                        // If a status index and pointer exist, proceed to fetch the status details.
                                        Block.each(Ops.EXPAND, "*statusIndexAndStatusPointer")
                                                .out("*statusIndex", "*statusPointer")
                                                .each(Ops.TUPLE, "*statusPointer").out("*statusPointers")
                                                .each(() -> new QueryFilterOptions(FilterContext.Thread, false))
                                                .out("*filterOptions")
                                                .invokeQuery("getStatusesFromPointers", "*requestAccountId",
                                                        "*statusPointers", "*filterOptions")
                                                .out("*statusQueryResults")
                                                .each((StatusQueryResults statusQueryResults) -> {
                                                    // Extracts the first (latest) status from the results, if any;
                                                    // otherwise, returns null.
                                                    if (statusQueryResults.results.size() == 0)
                                                        return null;
                                                    else
                                                        return new StatusQueryResult(statusQueryResults.results.get(0),
                                                                statusQueryResults.mentions);
                                                }, "*statusQueryResults").out("*lastStatus"))
                                // Fetches additional conversation details such as unread status and account IDs
                                // of participants.
                                .localSelect("$$accountIdToConvoIdToConvo",
                                ...
                                // Queries for account details of the participants using their IDs.
                                .invokeQuery("getAccountsFromAccountIds", null, "*accountIds").out("*accounts")
                                // Compiles the final conversation object with all details, including the latest
                                // status if available.
                                .each((Long cid, Boolean unread, List<AccountWithId> accounts,
        ...
        topologies.query("getAncestors", "*requestAccountId", "*childAuthorId", "*childStatusId", "*limit")
                ...
                // processing.
                .each((Long authorId, Long statusId) -> new StatusPointer(authorId, statusId), "*childAuthorId",
                        "*childStatusId")
                ...
                .loopWithVars(LoopVars.var("*count", 0)
                        ...
                        Block.ifTrue(new Expr(Ops.GREATER_THAN_OR_EQUAL, "*count", "*limit"),
                                // If the count reaches the limit, emit the collected status pointers and
                                // terminate the loop.
                                ...
                                Block.macro(extractFields("*statusPointer", "*authorId", "*statusId"))
                                        .select("$$accountIdToStatuses",
                                                Path.key("*authorId", "*statusId").view(Ops.FIRST))
                                        .out("*status")
                                        // If no status is found (null), emit the collected status pointers and
                                        // terminate the loop.
                                        .ifTrue(new Expr(Ops.IS_NULL, "*status"),
                                                Block.emitLoop("*statusPointers"),
                                                // If the status is not the original child status, increment the count
                                                // and add the status pointer to the collection.
                                                        ...
                                                        Block.each(Ops.INC, "*count").out("*nextCount")
                                                                .each((IPersistentList statusPointers,
                                                                        StatusPointer statusPointer) -> statusPointers
                                                                                .cons(statusPointer),
                                                                        "*statusPointers", "*statusPointer")
                                                                .out("*nextStatusPointers"),
                                                        // If it's the original status, do not modify the count or the
                                                        // collected pointers.
                                                        Block.each(Ops.IDENTITY, "*count").out("*nextCount")
                                                                .each(Ops.IDENTITY, "*statusPointers")
                                                                .out("*nextStatusPointers"))
                                                        // Check if the current status has a parent (indicating it's a
                                                        // reply).

                                                        .macro(extractFields("*status", "*content"))
                                                        .ifTrue(new Expr(Ops.IS_INSTANCE_OF, ReplyStatusContent.class,
                                                                "*content"),
                                                                // If so, extract the parent status pointer, prepare the
                                                                // next iteration variables,
                                                                // and continue the loop with the parent status pointer.
                                                                Block.macro(extractFields("*content", "*parent"))
                                                                        ...
                                                                        .continueLoop("*nextCount",
                                                                                "*nextStatusPointer",
                                                                                "*nextStatusPointers"),
                                                                // If there's no parent, emit the collected status
                                                                // pointers and terminate the loop.
        ...
        topologies.query("getDescendants", "*requestAccountId", "*parentAuthorId", "*parentStatusId", "*limit")
                .out("*results")
                .each((Long authorId, Long statusId) -> Arrays.asList(new StatusPointer(authorId, statusId)),
                        "*parentAuthorId", "*parentStatusId")
                .out("*initialStatusPointers")
                .loopWithVars(LoopVars.var("*count", 0)
                        .var("*statusPointerQueue", "*initialStatusPointers")
                        .var("*statusPointers", PersistentVector.EMPTY),
                        Block.each((List<StatusPointer> descendants) -> descendants.get(0), "*statusPointerQueue")
                                .out("*nextStatusPointer")
                                .ifTrue(new Expr(Ops.GREATER_THAN_OR_EQUAL, "*count", "*limit"),
                                        ...
                                        Block.macro(extractFields("*nextStatusPointer", "*authorId", "*statusId"))
                                                ...
                                                .ifTrue(new Expr(Ops.IS_NULL, "*nextStatus"),
                                                        ...
                                                        Block.ifTrue(
                                                                new Expr(Ops.NOT_EQUAL, "*statusId", "*parentStatusId"),
                                                                Block.each(Ops.INC, "*count").out("*nextCount")
                                                                        .each((PersistentVector statusPointers,
                                                                                Long authorId,
                                                                                Long statusId) -> statusPointers.cons(
                                                                                        new StatusPointer(authorId,
                                                                                                statusId)),
                                                                                "*statusPointers", "*authorId",
                                                                                "*statusId")
                                                                        .out("*nextStatusPointers"),
                                                                Block.each(Ops.IDENTITY, "*count").out("*nextCount")
                                                                        .each(Ops.IDENTITY, "*statusPointers")
                                                                ...
                                                                .localSelect("$$statusIdToRepliesById",
                                                                        Path.subselect(
                                                                                Path.key("*statusId")
                                                                                        .sortedMapRangeFrom(0L,
                                                                                                "*limit")
                                                                                        .mapVals()))
                                                                .out("*childStatusPointers")

                                                                // remove the first item from the list since we just
                                                                // processed it,
                                                                // then prepend its children to the list, so we get to
                                                                // them first (depth-first)
        ...
        topologies.query("getAccountMetadata", "*requestAccountId", "*accountId").out("*result")
                ...
                .each((Integer statusCount, Integer followerCount, Integer followeeCount, Boolean isFollowedByRequester,
                    ...
                    return metadata;
                        ...
                        "*statusCount", "*followerCount", "*followeeCount", "*isFollowedByRequester",
        ...
        topologies.query("getAccountIdToMetadata", "*requestAccountId", "*accountIds").out("*accountIdToMetadata")
                ...
                .each((List<List> accountIdAndMetadatas) -> {
                    ...
                    for (List accountIdAndMetadata : accountIdAndMetadatas) {
                        long accountId = (long) accountIdAndMetadata.get(0); // Extracts the account ID from the tuple.
                        AccountMetadata metadata = (AccountMetadata) accountIdAndMetadata.get(1); // Extracts the
                                                                                                  // metadata from the
                                                                                                  // tuple.
                        accountIdToMetadata.put(accountId, metadata); // Maps the account ID to its metadata.
                    }
        ...
        topologies.query("getAccountsFromAccountIds", "*requestAccountId", "*accountIds").out("*results")
                ...
                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*requestAccountId"),
                        Block.select("$$accountIdToSuppressions",
                                Relationships.isBlockedOrMutedPath("*requestAccountId", "*accountId", false))
                                .out("*isBlockedOrMuted")
                                .keepTrue(new Expr(Ops.NOT, "*isBlockedOrMuted"))) // Keeps the account only if it is
                ...
                .each((RamaFunction2<Integer, AccountWithId, IndexedAccountWithId>) IndexedAccountWithId::new, "*index",
                        "*accountWithId")
                ...
                .each((List<IndexedAccountWithId> unsortedResults) -> {
                    ...
                    return results.stream().map(o -> o.accountWithId).collect(Collectors.toList()); // Extracts and
                                                                                                    // returns sorted
                                                                                                    // AccountWithId
                                                                                                    // objects.
                }, "*unsortedResults").out("*results");
        ...
        topologies.query("getAccountsFromMentions", "*requestAccountId", "*statusResultWithIds").out("*results")
                ...
                .each(ApolloHelpers::getMentionsFromStatusResults, "*statusResultWithIds").out("*mentions")
        ...
        topologies.query("getStatusesFromPointers", "*requestAccountId", "*statusPointers", "*filterOptions")
                ...
                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*requestAccountId"),
                        Block.select("$$accountIdToFilterIdToFilter",
                                Path.key("*requestAccountId").subselect(Path.all())).out("*filterIdsAndFilters"),
                ...
                .ifTrue(new Expr(Ops.AND, "*excludeBlockedAndMuted",
                        ...
                        Block.select("$$accountIdToSuppressions",
                                Relationships.isBlockedOrMutedPath("*requestAccountId", "*authorId", false))
                                .out("*isBlockedOrMuted")
                                .keepTrue(new Expr(Ops.NOT, "*isBlockedOrMuted"))
                                .select("$$accountIdToSuppressions",
                                        Path.key("*authorId", "blocked").view(Ops.CONTAINS, "*requestAccountId")
                                                .filterPred(Ops.NOT)))
                ...
                // resolve the status
                .macro(extractFields("*status", "*content"))
                .macro(ApolloHelpers.resolveStatusResult("*requestAccountId", "*filters", "*filterContextValue",
                ...
                .ifTrue(new Expr(Ops.AND, "*excludeBlockedAndMuted",
                        ...
                        Block.macro(extractFields("*content", "*boosted"))
                                .each((StatusPointer boosted) -> boosted.authorId, "*boosted").out("*boostedAuthorId")
                                .select("$$accountIdToSuppressions",
                                        Relationships.isBlockedOrMutedPath("*requestAccountId", "*boostedAuthorId",
                                                false))
                                .out("*isBoostingBlockedOrMuted")
                                .keepTrue(new Expr(Ops.NOT, "*isBoostingBlockedOrMuted"))
                                .select("$$accountIdToSuppressions",
                                        Path.key("*boostedAuthorId", "blocked").view(Ops.CONTAINS, "*requestAccountId")
                                                .filterPred(Ops.NOT)))

                // get the poll data
                .ifTrue(new Expr(ApolloHelpers::statusResultHasPoll, "*statusResult"),
                        Block.each(ApolloHelpers::origAuthorId, "*authorId", "*status").out("*origAuthorId")
                                .each(ApolloHelpers::origStatusId, "*statusId", "*status").out("*origStatusId")
                                .hashPartition("*origAuthorId")
                                .localSelect("$$pollVotes",
                                        Path.key("*origStatusId")
                                                .subselect(
                                                        Path.multiPath(
                                                                Path.subselect(Path.key("choices").all()
                                                                        .collectOne(Path.first()).last()
                                                                        .view(Ops.SIZE)),
                                                                Path.key("allVoters")
                                                                        .multiPath(
                                                                                Path.view(Ops.SIZE),
                ...
                .ifTrue(new Expr(Ops.NOT_EQUAL, "*requestAccountId", "*authorId"),
                        ...
                        Block.ifTrue(new Expr(Ops.IS_NULL, "*requestAccountId"),
                                Block.keepTrue(
                                        new Expr(Ops.OR, new Expr(Ops.EQUAL, "*visibility", StatusVisibility.Public),
                                ...
                                Block.ifTrue(new Expr(Ops.EQUAL, "*visibility", StatusVisibility.Private),
                                        Block.select("$$followerToFollowees",
                                                Path.key("*requestAccountId", "*authorId")).out("*followee")
                                        ...
                                        Block.ifTrue(new Expr(Ops.EQUAL, "*visibility", StatusVisibility.Direct),
                                                Block.select("$$accountIdToAccount",
                                                        Path.key("*requestAccountId").filterPred(Ops.IS_NOT_NULL))
                                                        .out("*requestAccount")
                                                        .macro(ApolloHelpers.extractFields("*requestAccount", "*name"))
                                                        .each(Token::parseTokens, "*text").out("*tokens")
                                                        .each(Token::filterMentions, "*tokens").out("*mentions")
                                                        .each((Set<String> mentions, String accountName) -> mentions
                                                                .contains(accountName), "*mentions", "*name")
                                                        .out("*isMentioned")
                ...
                .each((List<IndexedStatusResultWithId> unsortedResults) -> unsortedResults.stream()
                        .flatMap(o -> ApolloHelpers.getAuthorIds(o.statusResultWithId.status).stream())
                ...
                .each((List<IndexedStatusResultWithId> unsortedResults,
                    ...
                    results.sort(
                            (IndexedStatusResultWithId a, IndexedStatusResultWithId b) -> (int) (a.index - b.index));
                    for (IndexedStatusResultWithId res : results) {
                        ApolloHelpers.updateAccountMetadata(res.statusResultWithId.status, accountIdToMetadata);
                    }
                ...
                .each(ApolloHelpers::getParentAccountIdsFromStatusResults, "*sortedResults").out("*parentAccountIds")
