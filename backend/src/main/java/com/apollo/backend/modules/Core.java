...
/*
 * This module implements the main parts of Apollo – timelines, statuses, and profiles. They're
...
public class Core implements RamaModule {
  ...
  public static class Timeline {
    ...
    public List<StatusPointer> readTimelineFrom(StatusPointer firstStatusPointer, StatusPointer endPointer, int maxAmt) {
      ...
      // - this is an optimization to deal with Apollo/soapbox design of paginating timeline using status ID
                 ...
                 .ifTrue(new Expr(Ops.EQUAL, Token.TokenKind.MENTION, "*kind"),
                         ...
                         Block.ifTrue(new Expr(Ops.EQUAL, Token.TokenKind.REMOTE_MENTION, "*kind"),
                                      Block.select("$$accountIdToAccount", Path.key(authorIdVar).filterPred(Ops.IS_NOT_NULL)).out("*author")
                                           ...
                                           .each(ApolloWebHelpers::sendRemoteStatus, FanoutAction.Add, "*author", statusIdVar, statusVar, "*parentStatus", "*boostedStatus", "*mainUrl", "*inboxUrl")));
    ...
    fan.source("*statusWithIdDepot").out("*microbatch")
       ...
       .atomicBlock(
           ...
           Block.hashPartition("$$followeeToRemoteServerToFollowers", "*authorId")
                ...
                .each(ApolloWebHelpers::sendRemoteStatus, new Expr(FanoutAction::findByValue, "*fanoutActionValue"), "*author", "*statusId", "*status", "*parentStatus", "*boostedStatus", null, "*sharedInboxUrl"))
       .atomicBlock(
           // fanout to remote mentions
           Block.each(ApolloHelpers::getRemoteMentionsFromStatus, "*status").out("*mentions")
                ...
                .keepTrue(new Expr(Ops.IS_NOT_NULL, "*accountContent"))
                .macro(extractFields("*accountContent", "*mainUrl", "*inboxUrl"))
                .each(ApolloWebHelpers::sendRemoteStatus, new Expr(FanoutAction::findByValue, "*fanoutActionValue"), "*author", "*statusId", "*status", "*parentStatus", "*boostedStatus", "*mainUrl", "*inboxUrl"))
      ...
      stream.source("*statusDepot").out("*initialData")
            ...
            .subSource("*data",
                SubSource.create(AddStatus.class)
                         ...
                         // Apollo actually does include DMs in this timeline, but soapbox does not,
                         ...
                         .ifTrue(new Expr(Ops.NOT_EQUAL, new Expr(ApolloHelpers::getStatusVisibility, "*status"), StatusVisibility.Direct),
  ...
  private void declareQueries(Topologies topologies) {
    ...
    topologies.query("getHomeTimeline", "*requestAccountId", "*firstStatusPointer", "*limit").out("*ret")
              ...
              .invokeQuery("getStatusesFromPointers", "*requestAccountId", "*statusPointers", "*filterOptions").out("*statusQueryResults")
              .each(ApolloHelpers::updateStatusQueryResults, "*statusQueryResults", "*statusPointers", "*limit", "*refreshed").out("*ret")
              .originPartition();
    ...
    topologies.query("getListTimeline", "*listId", "*firstTimelineIndex", "*limit").out("*results")
              ...
              .each(ApolloHelpers::updateStatusQueryResults, "*statusQueryResults", "*statusPointers", "*limit", false).out("*results")
              .originPartition();
    ...
    topologies.query("getStatusesFromPointers", "*requestAccountId", "*statusPointers", "*filterOptions").out("*results")
              ...
              // resolve the status
              .macro(extractFields("*status", "*content"))
              .macro(ApolloHelpers.resolveStatusResult("*requestAccountId", "*filters", "*filterContextValue", "*authorId", "*statusId", "*status", "*content", "*statusResult"))
              ...
              .each((List<IndexedStatusResultWithId> unsortedResults) ->
                  unsortedResults.stream().flatMap(o -> ApolloHelpers.getAuthorIds(o.statusResultWithId.status).stream()).collect(Collectors.toSet()),
