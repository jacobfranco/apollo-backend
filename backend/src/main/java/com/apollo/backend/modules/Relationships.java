...
import com.apollo.backend.navs.TField;
...
public class Relationships implements RamaModule {
  ...
  /*
   * These are constants that are sometimes adjusted in unit tests to ease
   * testing.
   */
  ...
  private static Filter updateFilter(Filter filter, EditFilter edit) {
    if (edit.isSetAction())
      filter.action = edit.getAction();
    if (edit.isSetContext())
      filter.contexts = edit.getContext();
    if (edit.isSetExpirationMillis())
      filter.expirationMillis = edit.getExpirationMillis();
    else
      filter.unsetExpirationMillis();
    if (edit.isSetTitle())
      filter.setTitle(edit.getTitle());
    if (edit.isSetKeywords()) {
      for (EditFilterKeyword keywordEdit : edit.getKeywords()) {
        if (keywordEdit.isSetAddKeyword()) {
          if (filter.getKeywords().size() < maxFilterClauses)
            filter.getKeywords().add(keywordEdit.getAddKeyword());
  ...
  private void declareStreamTopology(Topologies topologies) {
    ...
    // This PState is used to ensure duplicate client appends don't result in
    // undesired PState updates. For example,
    // a network partition of the client may cause it to retry and append the same
    // data again. This pattern
    ...
    // This utility from the rama-helpers project makes it extremely easy to
    // schedule future work
    // for a topology in a fault-tolerant way. This instance is used to expire
    // filters.
        ...
        .subSource("*data",
            SubSource.create(AddFilter.class)
                ...
                .ifTrue(new Expr(Ops.IS_NOT_NULL, "*expirationMillis"),
                    Block.macro(filterScheduler.scheduleItem("*expirationMillis",
                        new Expr(Ops.TUPLE, "*accountId", "*filterId"))))
                // Finally store the filter
                .localTransform("$$accountIdToFilterIdToFilter",
                    Path.key("*accountId", "*filterId").termVal("*filter")),
                ...
                .localTransform("$$accountIdToFilterIdToFilter",
                    Path.must("*accountId", "*filterId")
                        ...
                        .term((Set<StatusPointer> statuses, StatusPointer target) -> {
                          if (statuses.size() >= maxFilterClauses)
                            return statuses;
                          statuses.add(target);
                          return statuses;
                        }, "*target")),
    ...
    stream.source("*muteAccountDepot", StreamSourceOptions.retryAllAfter()).out("*data")
        .subSource("*data",
            SubSource.create(MuteAccount.class)
                ...
                .localSelect("$$accountIdToSuppressions", Path.key("*accountId", "muted").view(Ops.SIZE))
                .out("*muteeCount")
                .keepTrue(new Expr(Ops.LESS_THAN, "*muteeCount", relationshipCountLimit))
                .macro(extractFields("*options", "*expirationMillis"))
                .localTransform("$$accountIdToSuppressions",
                    Path.key("*accountId", "muted", "*targetId").termVal("*options"))
                .ifTrue(new Expr(MuteAccountOptions::isSetExpirationMillis, "*options"),
                    Block.macro(muteScheduler.scheduleItem("*expirationMillis",
                        new Expr(Ops.TUPLE, "*accountId", "*targetId")))),
    ...
    stream.pstate("$$accountIdToTargetIdToNote",
        PState.mapSchema(Long.class, PState.mapSchema(Long.class, String.class).subindexed()));
  ...
  public static AccountRelationshipQueryResult createAccountRelationship(long accountId, List<Boolean> bools,
      String note) {
   ...
   * - Determines for which users to recompute personalized who to follow
   * recommendations
   * - Selects next users on each partition, storing that state in the $$nextId
   * PState
   * - For 100M users and tick frequency of 30s, on 128 tasks this recomputes who
   * to follow
   * for each user once every 46 days.
   * - Also selects next users from the $$forceRecomputeUsers PState, which will
   * ensure
   * new users get personalized who to follow recommendations faster. Users are
   * added to
   * that PState when they cross the 10 and 100 follow thresholds.
   * - This then looks at all the follows of the follows for each of those users.
   * These are the
   * candidates for personalized who to follow recommendations. For each of those
   * candidates,
   * this code aggregates into a map the number of times they're followed by the
   * user's follows.
   */
  private SubBatch candidateCounts() {
    Block b = Block.allPartition()
        ...
        .localSelect("$$followerToFolloweesById",
            Path.sortedMapRangeFrom("*startId", WHO_TO_FOLLOW_USERS_PER_PARTITION))
        .out("*m")
        ...
        .each((PersistentHashSet s, OutputCollector collector) -> {
          ...
          while (it.hasNext() && ret.size() < 20)
            ret.add(it.next());
          for (Long accountId : ret)
            s = (PersistentHashSet) s.disjoin(accountId);
        ...
        .localSelect("$$followerToFolloweesById",
            Path.key("*accountId").sortedMapRangeFrom(0L, 200).mapVals().customNav(new TField("accountId")))
        .out("*followingId")
        .select("$$followerToFolloweesById",
            Path.key("*followingId").sortedMapRangeFrom(0L, 200).mapVals().customNav(new TField("accountId"))
                .filterNotEqual("*accountId"))
        .out("*candidateId")
        .hashPartition("*accountId")
  ...
  /*
   * This subbatch gets the unique set of users who received follows in this
   * microbatch. This is used
   * for updating the $$topFollowedUsers PState, which is used to augment
   * someone's who to follow
  ...
  /*
   * This filters a follow suggestion if they're already followed, have been
   * explicitly removed as a suggestion,
  ...
  private Block validSuggestionMacro(String accountIdVar, String targetIdVar, String resVar) {
    ...
    return Block.localSelect("$$followerToFollowees", Path.key(accountIdVar, targetIdVar)).out(idVar)
        .localSelect("$$removedFollowSuggestions", Path.key(accountIdVar)
            .view(Ops.CONTAINS, targetIdVar))
        .out(removedSuggestionVar)
        .localSelect("$$accountIdToSuppressions", Relationships.isBlockedOrMutedPath(accountIdVar, targetIdVar, false))
        .out(suppressedVar)
        .each(Ops.AND, new Expr(Ops.IS_NULL, idVar),
            new Expr(Ops.NOT, removedSuggestionVar),
            new Expr(Ops.NOT, suppressedVar),
            new Expr(Ops.NOT_EQUAL, targetIdVar, accountIdVar))
        .out(resVar);
  }
  ...
  public void define(Setup setup, Topologies topologies) {
    ...
    // This PState tracks how many times each user has followed other users
    // (irrespective of unfollows).
    // This is used for detecting follow thresholds for immediate recomputation of
    // personalized
    // recommendations.
    whoToFollow.pstate("$$followCounts", PState.mapSchema(Long.class, Integer.class));

    // This PState tracks the next starting account id for recomputes of
    // personalized recommendations.
    whoToFollow.pstate("$$nextId", Long.class).initialValue(0L).makePrivate();

    // This PState stores account ids that have recently passed the 10 or 100 follow
    // thresholds. It is
    ...
    // This portion of the topology consumes all Follow and Unfollow events to
    // produce the
    // $$followCounts and $$topFollowedUsers PStates.
    whoToFollow.source("*followAndBlockAccountDepot").out("*microbatch")
        // The Follow and Unfollow events in this microbatch are processed in batch to
        // determine
        // how much someone's follows have changed and whether that breaks the 10 or 100
        // follow
        // thresholds.
        .batchBlock(
            Block.subBatch(followCounts("*microbatch")).out("*accountId", "*count")
                ...
                .ifTrue(new Expr(Ops.LESS_THAN, "*currCount", whoToFollowThreshold2),
                    Block.each(Ops.PLUS, "*currCount", "*count").out("*newCount")
                        .ifTrue(
                            new Expr(Relationships::brokeThreshold, "*currCount", "*newCount", whoToFollowThreshold1,
                                whoToFollowThreshold2),
                            Block.localTransform("$$forceRecomputeUsers",
                                Path.filterPred((Set s) -> s == null || s.size() < 100).voidSetElem()
                                    .termVal("*accountId")))
        ...
        .batchBlock(
            Block.subBatch(followedUsers("*microbatch")).out("*accountId")
                ...
                .globalPartition()
                .agg("$$topFollowedUsers",
                    Agg.topMonotonic(numTopFollowedUsers, "*tuple").idFunction(Ops.FIRST).sortValFunction(Ops.LAST)));

    // This part of the topology does the recomputes of personalized recommendations
    // every 30s
        ...
        .batchBlock(
            Block.subBatch(candidateCounts()).out("*accountId", "*candidateCounts")
                ...
                .each((Map cc) -> {
                  ...
                  for (int i = 0; i < Math.min(1000, l.size()); i++)
                    ret.add(l.get(i).getKey());
                  return ret;
                ...
                .loop(
                    Block.ifTrue(
                        new Expr((List w, List c, Integer targetSuggestions) -> w.size() >= targetSuggestions
                            || c.size() == 0, "*whoToFollow", "*candidateOrder", numWhoToFollowSuggestions),
                        Block.emitLoop(),
    ...
    // This query topology fetches followers of an account that the requesting
    ...
    topologies.query("getFamiliarFollowers", "*yourAccountId", "*theirAccountId").out("*results")
        ...
        .localSelect("$$followerToFolloweesById",
            Path.key("*yourAccountId")
                ...
                .mapVals()
                .customNav(new TField("accountId")))
        .out("*yourFolloweeId")
        .select("$$followeeToFollowers", Path.key("*theirAccountId").must("*yourFolloweeId"))
        .originPartition()
        .agg(Agg.list("*yourFolloweeId")).out("*allResults")
        .each((List<Long> results) -> results.stream().limit(FAMILIAR_FOLLOWERS_LIMIT).collect(Collectors.toList()),
            "*allResults")
        .out("*results");

    // This query topology fetches metadata about the relationship between the two
    // accounts.
    topologies.query("getAccountRelationship", "*accountId", "*targetId").out("*relationship")
        ...
        .localSelect("$$accountIdToSuppressions", Path.key("*accountId", "muted").view(Ops.CONTAINS, "*targetId"))
        .out("*muting")
        ...
        .localSelect("$$followeeToNotifiedFollowerIds", Path.key("*targetId", "*accountId").view(Ops.IS_NOT_NULL))
        .out("*notifying")
        .each(Ops.IS_NOT_NULL, "*followedByIndex").out("*followedBy")
        .localSelect("$$accountIdToSuppressions", Path.key("*targetId", "blocked").view(Ops.CONTAINS, "*accountId"))
        .out("*blockedBy")
        .each(
        ...
        .each((AccountRelationshipQueryResult result, Follower followerPointer, Boolean notifying) -> {
          ...
          if (followerPointer == null)
            return result;
          if (followerPointer.isSetShowBoosts())
            result.setShowingBoosts(followerPointer.isShowBoosts());
          if (followerPointer.isSetLanguages())
            result.setLanguages(followerPointer.getLanguages());
          return result;
        }, "*relationship", "*followerPointer", "*notifying")
        .originPartition();

    // This query topology fetches the top followed users that the reuesting account
    // doesn't already follow.
    // It's a fallback set of suggestions when a user doesn't have enough
    // personalized suggestions (e.g.
    ...
    // This query topology gets follow suggestions for an account, filling the
    // suggestions
    ...
    if (testTopologiesHook != null)
      testTopologiesHook.invoke(topologies);
  }
