...
public class ApolloWebHelpers {
    ...
    public static String PUBLIC_URL = "https://www.w3.org/ns/activitystreams#Public"; // TODO: Maybe need to remove
    ...
    public static String getKeyId(Account account) {
        return String.format("acct:%s@%s", account.name, ApolloConfig.API_DOMAIN);
    }
    ...
    public static void addHtml(HashMap map, String originalContent, String receiverUrl) throws MalformedURLException {
        ...
        for (Token token : Token.parseTokens(originalContent)) {
            ...
            switch (token.kind) {
                ...
                case MENTION: {
                    String href = String.format("https://%s/@%s@%s", host, tokenContent, ApolloConfig.API_DOMAIN);
                    content += String.format("<a href=\"%s\">%s@%s</a>", href, tokenContent, ApolloConfig.API_DOMAIN);
                    // update the list of tags
                    List<HashMap> tags = (List<HashMap>) map.getOrDefault("tag", new ArrayList<HashMap>());
                    tags.add(new HashMap() {{
                        put("type", "Mention");
    ...
    public static boolean sendRemoteStatus(FanoutAction fanoutAction, Account sender, long statusId, Status status, Status parentStatus, Status boostedStatus, String mainUrl, String inboxUrl) {
        ...
        StatusVisibility visibility = ApolloHelpers.getStatusVisibility(status);
        String followersUrl = String.format("%s/users/%s/followers", ApolloConfig.API_URL, sender.name);
        ...
        String id = String.format("%s/users/%s/statuses/%s", ApolloConfig.API_URL, sender.name, ApolloHelpers.serializeStatusPointer(new StatusPointer(status.authorId, statusId)));
        String actorUrl = String.format("%s/users/%s", ApolloConfig.API_URL, sender.name);
            ...
            case Edit:
            {
                ...
                if (boostedStatus != null) {
                    ...
                    if (objectUrl == null) {
                        ...
                        objectUrl = ApolloConfig.API_URL + "/users/" + sender.name + "/statuses/" + ApolloHelpers.serializeStatusPointer(statusContent.boosted);
                    }
                    ...
                    if (status.content.isSetNormal()) {
                        ...
                        if (remoteParentUrl == null) {
                            remoteParentUrl = String.format("%s/users/%s/statuses/%s", ApolloConfig.API_URL, sender.name, ApolloHelpers.serializeStatusPointer(content.parent));
                        }
