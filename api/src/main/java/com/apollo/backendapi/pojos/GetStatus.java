...
public class GetStatus {
    ...
    public GetStatus(StatusPointer statusPointer, StatusResult status, Map<String, AccountWithId> mentions) {
        ...
        if (status.content.isSetNormal()) {
            ...
            this.repost = new GetStatus(new StatusPointer(content.status.author.accountId, content.statusId), content.status, mentions);
        }

        if (status.content.isSetBoost()) this.filtered = this.repost.filtered;
        else {
        ...
        for (Token token : Token.parseTokens(originalContent)) {
            ...
            switch (token.kind) {
                ...
                case LINK:
                    ...
                    break;
                case HASHTAG:
                    this.content += String.format("<a href=\"%s/tags/%s\">%s</a>", ApolloConfig.FRONTEND_URL, tokenContent, tokenContent);
                    this.tags.add(new GetTag(tokenContent));
                    break;
                case MENTION:
                    if (mentions.containsKey(tokenContent)) {
                        this.content += String.format("<a href=\"%s/@%s\">%s</a>", ApolloConfig.FRONTEND_URL, tokenContent, tokenContent);
