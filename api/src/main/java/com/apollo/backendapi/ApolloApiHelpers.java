...
public class ApolloApiHelpers {
    ...
    public static CompletableFuture<GetActivityPubProfile> getProfile(String nameWithHost) {
        ...
        if (parts.length == 2) {
            final String url;
            if (ApolloConfig.API_DOMAIN.equals("localhost")) url = String.format("%s/users/%s", ApolloConfig.API_URL, parts[0]);
            ...
            switch (token.kind) {
                ...
                case MENTION:
                    ...
                    if (host != null && !host.isEmpty()) {
                        if (host.equals(ApolloConfig.API_DOMAIN)) content += token.content;
                ...
                case REMOTE_MENTION:
                    ...
                    if (parts.length == 2 && parts[1].equals(ApolloConfig.API_DOMAIN)) content += parts[0];
    ...
    public static PostStatus createPostStatus(GetActivityPubProfile profile, HashMap object) {
        ...
        if (to.contains(ApolloWebHelpers.PUBLIC_URL)) postStatus.visibility = "public";
        else if (to.contains(profile.followers)) {
            if (cc.contains(ApolloWebHelpers.PUBLIC_URL)) postStatus.visibility = "unlisted";
    ...
    public static PostStatus createPostStatus(GetActivityPubProfile profile, HashMap object, StatusQueryResult parentStatus) {
        ...
        postStatus.in_reply_to_id = ApolloHelpers.serializeStatusPointer(new StatusPointer(parentStatus.result.status.author.accountId, parentStatus.result.statusId));
        return postStatus;
    }
    ...
    public static GetActivityPubProfile createProfile(AccountWithId accountWithId) {
        ...
        profile.id = String.format("%s/users/%s", ApolloConfig.API_URL, name);
        profile.type = "Person";
        profile.following = String.format("%s/users/%s/following", ApolloConfig.API_URL, name);
        profile.followers = String.format("%s/users/%s/followers", ApolloConfig.API_URL, name);
        profile.inbox = String.format("%s/users/%s/inbox", ApolloConfig.API_URL, name);
        profile.outbox = String.format("%s/users/%s/outbox", ApolloConfig.API_URL, name);
        profile.featured = String.format("%s/users/%s/collections/featured", ApolloConfig.API_URL, name);
        profile.featuredTags = String.format("%s/users/%s/collections/tags", ApolloConfig.API_URL, name);
        ...
        profile.url = String.format("%s/@%s", ApolloConfig.API_URL, name);
        ...
        profile.devices = String.format("%s/users/%s/collections/devices", ApolloConfig.API_URL, name);
        profile.attachment = new ArrayList<>();
        if (accountWithId.account.content.isSetLocal()) {
            profile.publicKey = new GetActivityPubProfile.PublicKey(ApolloWebHelpers.getKeyId(accountWithId.account), profile.id, accountWithId.account.publicKey);
        }
        ...
        profile.endpoints = new HashMap(){{ put("sharedInbox", String.format("%s/inbox", ApolloConfig.API_URL)); }};
        return profile;
    }
    ...
    public static List<EditAccountField> getEditsFromProfile(AccountWithId accountWithId, GetActivityPubProfile profile) {
       ...
       if (bio != null) edits.add(EditAccountField.bio(ApolloApiHelpers.cleanHtmlTags(bio)));
