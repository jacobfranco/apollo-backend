...
public class ApolloApiSchedulingConfig {
    ...
    public static void refreshHomeTimelineProxies() {
        if (ApolloApiController.manager == null) return; // exit early if the manager hasn't been initialized yet
        ...
        for(StreamState ss: SESSION_ID_TO_STATE.values()) {
            for (ProxyState<SortedMap> proxy : ss.proxies) {
                if (proxy instanceof ApolloApiManager.HomeTimelineProxyState) proxies.add((ApolloApiManager.HomeTimelineProxyState) proxy);
            }
        }
        if(!proxies.isEmpty()) ApolloApiController.manager.refreshHomeTimelineProxies(proxies);
    }
    ...
    public static void refreshGlobalTimelines(int cacheSize, int queryLimit, boolean fullRefresh) {
        ...
        for (Map.Entry<GlobalTimeline, ConcurrentSkipListMap<Long, StatusQueryResult>> streamEntry : GLOBAL_TIMELINE_TO_INDEX_TO_STATUS.entrySet()) {
            ...
            SortedMap<Long, StatusQueryResult> indexToStatusNew = ApolloApiController.manager.getGlobalTimeline(timeline, limit);
