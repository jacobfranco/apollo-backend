...
public class ApolloApiRateLimiter implements WebFilter {
    private static final int BUCKET_COUNT = 1000;

    private static final int MAIN_REQUESTS_PER_SECOND = 100;
    ApolloApiConcurrentFixedMap<String, Bucket> mainBuckets = new ApolloApiConcurrentFixedMap<>(BUCKET_COUNT);

    private static final int LOGIN_REQUESTS_PER_SECOND = 2;
    ApolloApiConcurrentFixedMap<String, Bucket> loginBuckets = new ApolloApiConcurrentFixedMap<>(BUCKET_COUNT);
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        if (remoteAddr != null) {
            ...
            final ApolloApiConcurrentFixedMap<String, Bucket> buckets;
            final int reqPerSec;
            if (request.getMethod() == HttpMethod.POST
                    && request.getPath().pathWithinApplication().value().equals("/oauth/token")) {
            ...
            if (!bucket.tryConsume(1))
                return Mono.error(new ResponseStatusException(HttpStatus.BANDWIDTH_LIMIT_EXCEEDED));
        }
